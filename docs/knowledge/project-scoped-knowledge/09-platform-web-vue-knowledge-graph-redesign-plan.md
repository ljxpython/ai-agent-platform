# platform-web-vue 知识图谱前端重构方案

> Superseded: 本文已被 `10-platform-web-vue-knowledge-graph-single-milestone-plan.md` 取代，后者是当前有效的单阶段实施计划。本文保留仅用于历史参考。

## 1. 文档目的

本文只讨论一件事：

> `apps/platform-web-vue` 中知识图谱页应该如何从当前的手写 SVG 实现，重构为可维护、可扩展、可验证的正式图谱前端。
...
- `apps/platform-web-vue/docs/frontend-visual-baseline-standard.md`
...
推荐在 `apps/platform-web-vue/src/modules/knowledge/` 下新增图谱专属分层：
...
- [ ] 为 `apps/platform-web-vue` 增加 Sigma 与 graphology 依赖
- [ ] 确认类型支持与构建链兼容
- [ ] 确认无新增不必要的大型 UI 依赖
...
- `platform-web-vue` 知识图谱页采用 **Vue + Sigma + graphology** 重构
