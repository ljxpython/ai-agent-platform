...
- **状态：** 源码调研和方案已形成，待人工评审；本轮未改业务代码、数据库或部署。
...
本轮交付是跨仓库详细计划。评审须覆盖授权回调与平台 operation 映射、创建补偿、任务身份有效期、幂等域、Store namespace 契约及数据回退。平台 AGENTS.md 要求治理改动“方案评审（人工）→批准→实施”，批准记录写在主方案 README。

现有服务已完成能力的状态不因本次规划改写。本专项尚未执行功能、真实数据库、浏览器、真实模型或迁移测试；每个专题包含独立 Phase 验证和最终联合准入要求。
