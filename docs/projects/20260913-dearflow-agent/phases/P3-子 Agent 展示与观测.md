...
2026-09-15后续交付：通用Server官方对齐与平台显式v3试用已在[独立项目](../../20260915-graphharbor-v3-alignment/README.md)完成后端，当前平台安装post30；代码路径、真实失败／恢复／文件／研究／双子任务与观测证据见该项目03实施记录。F3前端仍仅交接，默认切换须浏览器验收后另行实施，不将后端升级标作F3完成。
...
| 结果与状态核验 | 区分成功、失败、中断和取消；结果引用可读取，无证据不能声称产物完成 | 05；03 证据 | 后端 done：组合成功／权限拒绝／父取消及真实双任务结果、checkpoint回读通过；v3生命周期回放通过，未新增结构化结果验收框架 |
| 断线和父 Run 取消 | 刷新、缺失 discovery、断流后从引擎恢复事实；父取消不能把未完成任务显示成功 | 05/S-C | partial：真实父取消至 interrupted 通过；浏览器断流／刷新 deferred |
| 已有 tracing 与用量 | 复用现有字段，保留父子关联；未知用量显示未知，不估造子任务费用 | 05/S-D | 后端 done：受信字段修复、callback与消息usage通过；真实Langfuse导出及两个子GENERATION父关联／实际用量回读通过；展示 deferred |
...
| 4. 父取消与恢复 | 沿用现有父 Run 取消；验证取消竞态、断流、刷新与超时，不把 ACK／流结束视为子任务成功 | `apps/runtime-service/tests/services/dearflow_agent/test_platform.py`；官方 Run／子图状态 | post28真实父取消1项通过（3.36s）、组合传播通过；v2/v3双任务成功和checkpoint回读通过；浏览器恢复后置 |
| 5. 已有观测与用量 | 核对 parent span／namespace／调用 ID；仅在现有信息足够时展示输入／输出用量，缺失标未知，避免父级总量与子级重复相加 | `apps/runtime-service/src/runtime_service/observability/langfuse.py` 与对应测试 | 4个受信字段已修；真实Langfuse两子GENERATION及父关联、usage回读1项通过（10.39s），见08 |
| 6. 收口 | 后端并发／权限／取消回归；前端落实后从 Dear 路由做刷新、同名任务及失败状态验收 | 本执行包与08实施记录，每项列完整修改路径和实际命令 | 后端39项回归已有证据；平台请求21＋适配器13项通过；v3真实成功7.74s、v2成功10.89s、父取消3.36s。Server151 passed/18 skipped；历史延迟根因未证实，性能优化归因撤回，F3 deferred |
...
- [x] 发布安装GraphHarbor post28；平台显式v3透传与非法版本校验；真实双任务回放、父取消、外部观测回读通过。默认v2保持兼容。
...
- [x] 本轮后端使用确定性测试和真实平台链路验证；浏览器与生产稳定性验收另列，不计入通过。
- [x] 未实现能力标记 deferred／partial，不用模拟结果勾选完成。

## 状态

partial：本轮约定后端范围 done，真实v2/v3双任务、生命周期回放、父取消与外部观测已通过；F3前端与浏览器联合验收 deferred，历史延迟根因和生产稳定性门禁仍未关闭。逐项修改文件、函数、测试命令和Run标识见 [08 收口记录](../implementation/08-p3-server-verification-closeout.md)，07保留历史失败证据及纠正说明。独立子取消／完整用量账本不在本次范围，未宣称整阶段或生产联合验收完成。
