...
**开始处理任何需求前，AI 主动读取 `docs/CONTEXT.md` 同步项目现状。**
...
- `docs/lessons/index.md` — 快速索引，不确定读哪个时先看这里
- `docs/lessons/{service-name}.md` — 该服务踩过的坑（`runtime-service`、`platform-api` 等）
- `docs/lessons/ai-workflow.md` — 处理 AI 工作流/Harness 本身改动时读取

**不需要全部加载**，只读和当前任务直接相关的文件。
...
用户问"这个方案是否可行"、"要不要这样设计"、"帮我分析一下 xxx"这类问题时，直接对话给出分析和结论，不调用任何 Skill。
...
> 这里只放普适性反模式（任何场景都不能做的）。条数上限 10 条——超了说明领域经验没有沉淀到 `docs/lessons/` 里。

- ❌ 不得在未建 `tasks.md` 的情况下开始写代码（链路/治理改动）
