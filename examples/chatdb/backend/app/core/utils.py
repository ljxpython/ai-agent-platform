...
def format_schema_context(schema_context: Dict[str, Any]) -> str:
    ...
    for table in schema_context["tables"]:
        ...
        # 添加列信息
        columns = [
            col for col in schema_context["columns"] if col["table_name"] == table_name
        ]
...
def format_schema_as_markdown(schema_context: Dict[str, Any]) -> str:
    ...
    for table in schema_context["tables"]:
        ...
        if columns:
            ...
            for column in columns:
                ...
                schema_info += (
                    f"| {column['name']} | {column['type']} | {pk} | {fk} | {desc} |\n"
                )
        schema_info += "\n"
    ...
    if schema_context.get("relationships"):
        ...
        for rel in schema_context["relationships"]:
            rel_type = (
                f" ({rel['relationship_type']})" if rel.get("relationship_type") else ""
            )
