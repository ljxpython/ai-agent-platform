...
def get_db_engine(connection: DBConnection, password: str = None):
    ...
    try:
        ...
        if connection.db_type.lower() == "mysql":
            ...
            print(
                f"Connecting to MySQL database: {connection.host}:{connection.port}/{connection.database_name}"
            )
            ...
            print(
                f"Connecting to PostgreSQL database: {connection.host}:{connection.port}/{connection.database_name}"
            )
            return create_engine(conn_str)
...
def test_db_connection(connection: DBConnection) -> bool:
    ...
    try:
        print(
            f"Testing connection to {connection.db_type} database at {connection.host}:{connection.port}/{connection.database_name}"
        )
