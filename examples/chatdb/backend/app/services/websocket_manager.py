...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: Optional[str] = None) -> str:
        ...
        logger.info(
            f"新的WebSocket连接已建立: connection_id={connection_id}, user_id={user_id}"
        )
    ...
    async def disconnect(self, connection_id: str) -> None:
        ...
        logger.info(
            f"WebSocket连接已关闭: connection_id={connection_id}, user_id={user_id}"
        )
    ...
    async def broadcast(
        ...
        exclude = exclude or []
        connection_ids = [
            cid for cid in self.active_connections.keys() if cid not in exclude
        ]
    ...
    async def get_feedback(
        self, connection_id: str, timeout: float = 60.0
            ...
            while connection_id in self.active_connections:
                ...
                if connection_id in self.active_connections:
                    ...
                    if elapsed > self.session_timeout:
                        logger.info(
                            f"连接超时，准备关闭: {connection_id}, 最后活动: {elapsed}秒前"
                        )
                        await self.disconnect(connection_id)
                        break
