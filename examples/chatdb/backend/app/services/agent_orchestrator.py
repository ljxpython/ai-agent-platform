...
from app.agents.types import DEFAULT_DB_TYPE, TopicTypes
...
class AgentOrchestrator:
    ...
    def _setup_database_connection(
        ...
        if connection_id:
            ...
            if connection_info:
                print(
                    f"[连接ID: {connection_id}] 数据库类型: {connection_info.db_type}"
                )
                ...
                try:
                    ...
                    if connection_info.db_type.lower() == "mysql":
                        ...
                        print(
                            f"不支持的数据库类型: {connection_info.db_type}，使用默认连接"
                        )
    ...
    async def process_query(
        ...
        try:
            ...
            await factory.register_all_agents(
                runtime, collector, user_feedback_enabled=user_feedback_enabled
            )
            ...
            # 发送初始消息
            message = QueryMessage(query=query, connection_id=connection_id)

            # 发送到表结构检索智能体
            await runtime.publish_message(
                message, topic_id=DefaultTopicId(type=TopicTypes.SCHEMA_RETRIEVER.value)
            )
