    ...
    if value_mappings:
        ...
        for column, mappings in value_mappings.items():
            ...
            for nl_term, db_value in mappings.items():
                mappings_str += (
                    f"--   自然语言中的'{nl_term}'指数据库中的'{db_value}'\n"
                )
        mappings_str += "\n"
...
def process_text2sql_query(
    ...
    try:
        ...
        schema_context = retrieve_relevant_schema(
            db, connection.id, natural_language_query
        )
        ...
        prompt = construct_prompt(
            schema_context, natural_language_query, value_mappings
        )
        ...
        return QueryResponse(
            sql="", results=None, error=f"处理查询时出错: {str(e)}", context=None
        )
