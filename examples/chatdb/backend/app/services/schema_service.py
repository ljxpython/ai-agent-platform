...
def discover_schema(connection: DBConnection) -> List[Dict[str, Any]]:
    ...
    try:
        print(
            f"Discovering schema for {connection.name} ({connection.db_type} at {connection.host}:{connection.port}/{connection.database_name})"
        )
...
def discover_generic_schema(inspector) -> List[Dict[str, Any]]:
    ...
    for table_name in tables:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Warning: Could not get primary keys for {table_name}: {str(pk_error)}"
                )
                ...
                for column in table_info["columns"]:
                    ...
                    if (
                        ...
                        or col_name == f"{table_name.lower()}_id"
                    ):
                        if (
                            "int" in column["data_type"].lower()
                            or "serial" in column["data_type"].lower()
                        ):
                            print(
                                f"Identified potential primary key by naming convention: {column['column_name']}"
                            )
                            column["is_primary_key"] = True
            ...
            try:
                    ...
                    for i, constrained_column in enumerate(fk["constrained_columns"]):
                        for column in table_info["columns"]:
                            if column["column_name"] == constrained_column:
                                ...
                                # 确保引用列索引有效
                                referred_column_idx = (
                                    min(i, len(fk["referred_columns"]) - 1)
                                    if fk["referred_columns"]
                                    else 0
                                )
                                referred_column = (
                                    fk["referred_columns"][referred_column_idx]
                                    if fk["referred_columns"]
                                    else "id"
                                )
                                column["references"] = {
                                    ...
                                    "constraint_name": fk.get(
                                        "name", ""
                                    ),  # 保存约束名称
                                    "is_part_of_composite_key": len(
                ...
                print(
                    f"Warning: Could not get foreign keys for {table_name}: {str(fk_error)}"
                )
                ...
                for column in table_info["columns"]:
                    ...
                    if col_name.endswith("_id") and not column["is_primary_key"]:
                        ...
                        if table_exists:
                            print(
                                f"Identified potential foreign key by naming convention: {column['column_name']} -> {potential_table}"
                            )
                            column["is_foreign_key"] = True
                            column["references"] = {
                                "table": next(
                                    t for t in tables if t.lower() == potential_table
                                ),
                                "column": "id",  # 假设主键是 'id'
                    ...
                    # 如果列名与其他表名相同，也可能是外键
                        ...
                        for table_name_to_check in tables:
                            if (
                                col_name == table_name_to_check.lower()
                                or col_name == f"{table_name_to_check.lower()}id"
                            ):
                                print(
                                    f"Identified potential foreign key by table name match: {column['column_name']} -> {table_name_to_check}"
                                )
                                column["is_foreign_key"] = True
                                column["references"] = {
            ...
            try:
                ...
                print(
                    f"Warning: Could not get unique constraints for {table_name}: {str(uc_error)}"
                )
            ...
            try:
                ...
                print(
                    f"Warning: Could not get indexes for {table_name}: {str(idx_error)}"
                )

        except Exception as column_error:
            print(
                f"Warning: Could not process columns for {table_name}: {str(column_error)}"
            )
            continue
    ...
    print(
        f"Schema discovery completed successfully. Found {len(schema_info)} tables/views."
    )
    return schema_info
            ...
            for column in columns:
                column_info = {
                    ...
                    "is_primary_key": column.get(
                        "primary_key", False
                    ),  # SQLite provides primary_key info directly
                    "is_foreign_key": False,
                    "is_nullable": column.get("nullable", True),
                }
                table_info["columns"].append(column_info)
...
def save_discovered_schema(
    db: Session, connection_id: int, schema_info: List[Dict[str, Any]]
    ...
    for table_info in schema_info:
        for column_info in table_info["columns"]:
            if column_info.get("is_foreign_key") and column_info.get("references"):
                ...
                print(
                    f"Processing relationship: {source_table_name}.{source_column_name} -> {target_table_name}.{target_column_name}"
                )
...
def sync_schema_to_graph_db(connection_id: int):
    ...
    try:
        ...
        # Connect to Neo4j
        print(
            f"Connecting to Neo4j at {settings.NEO4J_URI} with user {settings.NEO4J_USER}"
        )
        ...
        with driver.session() as session:
            ...
            try:
                ...
                tables = crud.schema_table.get_by_connection(
                ...
                if len(tables) == 0:
                    print(
                        f"Warning: No tables found for connection_id: {connection_id}"
                    )
                    return False
                ...
                for table in tables:
                    print(
                        f"Creating Table node for table: {table.table_name} (id: {table.id})"
                    )
                    session.run(
                        """
                        CREATE (t:Table {
                    ...
                    for column in columns:
                        print(
                            f"Creating Column node for column: {column.column_name} (id: {column.id})"
                        )
                        session.run(
                            """
                            CREATE (c:Column {
                ...
                relationships = crud.schema_relationship.get_by_connection(
                    db=db, connection_id=connection_id
                )
                print(
                    f"Found {len(relationships)} relationships for connection_id: {connection_id}"
                )

                for rel in relationships:
                    print(
                        f"Creating REFERENCES relationship from column id: {rel.source_column_id} to column id: {rel.target_column_id}"
                    )
                    session.run(
                        """
                ...
                print(
                    f"Successfully synced schema to Neo4j for connection_id: {connection_id}"
                )
            finally:
                db.close()
