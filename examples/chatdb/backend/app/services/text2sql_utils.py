...
async def analyze_query_with_llm(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if json_match:
            ...
            # 验证必需字段
            if not all(
                k in analysis for k in ["entities", "relationships", "query_intent"]
            ):
...
async def find_relevant_tables_semantic(
    query: str, query_analysis: Dict[str, Any], all_tables: List[Dict[str, Any]]
...
def basic_table_matching(
    query: str, all_tables: List[Dict[str, Any]]
) -> List[Tuple[int, float]]:
    """
    基本关键词匹配回退方法
    """
...
async def filter_expanded_tables_with_llm(
    query: str,
    ...
    try:
        ...
        if json_match:
            ...
            # 获取应包含的表的ID
            include_ids = [
                t["table_id"] for t in filtered_tables if t.get("include", False)
            ]
...
def format_schema_for_prompt(schema_context: Dict[str, Any]) -> str:
    ...
    for table in tables:
        ...
        if table_name in columns_by_table:
            for column in columns_by_table[table_name]:
                ...
                schema_str += (
                    f"--   {col_name} {col_type}{pk_flag}{fk_flag}{col_desc}\n"
                )
    ...
    if relationships:
        ...
        for rel in relationships:
            rel_type = (
                f" ({rel['relationship_type']})" if rel["relationship_type"] else ""
            )
...
def get_value_mappings(
    db: Session, schema_context: Dict[str, Any]
...
def process_sql_with_value_mappings(
    sql: str, value_mappings: Dict[str, Dict[str, str]]
) -> str:
    """
...
async def retrieve_relevant_schema(
    db: Session, connection_id: int, query: str
) -> Dict[str, Any]:
    """
    基于自然语言查询检索相关的表结构信息
    ...
    try:
        ...
        with driver.session() as session:
            ...
            # 3. 使用语义搜索基于查询分析找到相关表
            relevant_table_ids = await find_relevant_tables_semantic(
                query, query_analysis, all_tables
            )
            ...
            for entity in query_analysis["entities"]:
                ...
                for record in result:
                    ...
                    # 添加表或更新（如果已存在且有更好的描述）
                    if (
                        record["table_id"] not in relevant_tables_dict
                        or not relevant_tables_dict[record["table_id"]][2]
                    ):
                    ...
                    # 为有匹配列的表增加相关性分数
                    table_relevance_scores[record["table_id"]] = (
                        table_relevance_scores.get(record["table_id"], 0) + 0.5
                    )
                ...
                if table_ids:
                    ...
                    for record in result:
                        ...
                        if record["id"] not in relevant_tables_dict or (
                            not relevant_tables_dict[record["id"]][2]
                        ...
                        # 相关表基于源表的分数获得相关性分数
                        source_score = table_relevance_scores.get(
                            record["source_table_id"], 0
                        )
                        table_relevance_scores[record["id"]] = (
                            source_score * 0.7
                        )  # 相关表分数降低

                # 7. 使用LLM评估扩展表是否真正与查询相关
                expanded_tables = [
                    t for t in relevant_tables_dict.values() if t[0] not in table_ids
                ]
                if expanded_tables:
                    ...
                    # 移除LLM认为不相关的表
                    # 只保留相关表
                    filtered_table_ids = set(table_ids).union(
                        {t[0] for t in filtered_expanded_tables}
                    )
                    relevant_tables_dict = {
                        tid: t
                        for tid, t in relevant_tables_dict.items()
                        if tid in filtered_table_ids
                    }
        ...
        # 转换为字典列表
        tables_list = [
            {"id": t[0], "name": t[1], "description": t[2]} for t in sorted_tables
        ]

        # 如果没有找到相关表，返回所有表
        if not tables_list:
            all_tables_from_db = crud.schema_table.get_by_connection(
        ...
        # 如果返回所有表，则获取所有关系
        all_tables_count = len(
            crud.schema_table.get_by_connection(db=db, connection_id=connection_id)
        )
        if len(tables_list) == all_tables_count:
            all_relationships = crud.schema_relationship.get_by_connection(
                db=db, connection_id=connection_id
            )

            for rel in all_relationships:
                source_table = next(
                    (t for t in tables_list if t["id"] == rel.source_table_id), None
                )
                target_table = next(
                    (t for t in tables_list if t["id"] == rel.target_table_id), None
                )
                source_column = next(
                    (c for c in columns_list if c["id"] == rel.source_column_id), None
                )
                target_column = next(
                    (c for c in columns_list if c["id"] == rel.target_column_id), None
                )
            ...
            for table in tables_list:
                source_rels = crud.schema_relationship.get_by_source_table(
                    db=db, source_table_id=table["id"]
                )
                target_rels = crud.schema_relationship.get_by_target_table(
                    db=db, target_table_id=table["id"]
                )

                for rel in source_rels + target_rels:
                    # 只包含相关表集中的表之间的关系
                    if (
                        rel.source_table_id in table_ids
                        and rel.target_table_id in table_ids
                    ):
                        source_table = next(
                            (t for t in tables_list if t["id"] == rel.source_table_id),
                            None,
                        )
                        target_table = next(
                            (t for t in tables_list if t["id"] == rel.target_table_id),
                            None,
                        )
                        source_column = next(
                            (
