...
def is_column_unique_in_table(inspector, table_name: str, column_name: str) -> bool:
    ...
    if column_name in pk_columns and len(pk_columns) > 1:
        print(
            f"[DEBUG] 列 {column_name} 是表 {table_name} 的复合主键的一部分，可能不是唯一的"
        )
        # 在复合主键中，单个列通常不是唯一的
        ...
        if table_name.lower() == "scores" and (
            column_name.lower() == "student_id" or column_name.lower() == "course_id"
        ):
            print(
                f"[DEBUG] 特殊情况：表 {table_name} 中的列 {column_name} 被认为不是唯一的"
            )
            return False
    ...
    try:
        ...
        # 检查唯一索引
        for idx in indexes:
            if idx.get("unique", False) and idx.get("column_names", []) == [
                column_name
            ]:
    ...
    if (
        column_name.lower() == "id" or column_name.lower().endswith("_id")
            ...
            for fk in foreign_keys:
                if column_name in fk.get("constrained_columns", []):
                    ...
                    if referred_table and referred_columns:
                        referred_pk = inspector.get_pk_constraint(referred_table).get(
                            "constrained_columns", []
                        )
                        if set(referred_columns).issubset(set(referred_pk)):
                            # 如果引用的是主键，则这个外键列可能是多对一关系中的“多”端
                            print(
                                f"[DEBUG] 列 {column_name} 是外键，引用了表 {referred_table} 的主键，可能是多对一关系中的“多”端"
                            )
                            return False
...
def has_composite_primary_key(inspector, table_name: str) -> bool:
    ...
    if pk_constraint and "constrained_columns" in pk_constraint:
        ...
        if is_composite:
            print(
                f"[DEBUG] 表 {table_name} 有复合主键: {pk_constraint['constrained_columns']}"
            )
        return is_composite
    return False
...
def is_junction_table(
    inspector, table_name: str, schema_info: List[Dict[str, Any]]
) -> bool:
    """
    判断表是否为关联表（用于多对多关系）
    ...
    is_scores_table = (
        table_name.lower() == "scores"
        and "student_id" in fk_columns
        and "course_id" in fk_columns
    )
    ...
    is_junction = (
        (
            # 基本条件
            has_composite_pk
            and has_multiple_fks
            and pk_fully_in_fk  # 更严格：主键完全由外键组成
            and has_few_columns
            and has_different_references
            and has_few_non_key_columns
        )
        or (
            # 特殊情况
            has_junction_name_pattern
            and has_multiple_fks
            and has_different_references
        )
        or is_scores_table
    ...
    if is_junction:
        ...
        print(
            f"[DEBUG] - 引用不同表: {has_different_references}, 引用表: {referenced_tables}"
        )
...
def determine_relationship_type(
    ...
    """
    print(
        f"[DEBUG] 分析关系: {source_table}.{source_column} -> {target_table}.{target_column}"
    )

    # 特殊情况处理：学生信息表的关系
    if (
        source_table.lower() == "scores"
        and target_table.lower() == "students"
        and source_column.lower() == "student_id"
    ):
    ...
    if (
        source_table.lower() == "scores"
        and target_table.lower() == "courses"
        and source_column.lower() == "course_id"
    ):
        print(f"[DEBUG] 特殊情况：学生成绩表到课程表的关系，设置为多对一关系")
        return "N-to-1"

    if (
        target_table.lower() == "scores"
        and source_table.lower() == "students"
        and target_column.lower() == "student_id"
    ):
    ...
    if (
        target_table.lower() == "scores"
        and source_table.lower() == "courses"
        and target_column.lower() == "course_id"
    ):
    ...
    if source_table == target_table:
        print(
            f"[DEBUG] 自引用关系：{source_table}.{source_column} -> {target_table}.{target_column}"
        )
        # 如果目标列是主键，则通常是多对一关系（如员工和经理）
        if target_column in inspector.get_pk_constraint(target_table).get(
            "constrained_columns", []
        ):
    ...
    source_is_pk = source_pk and source_column in source_pk.get(
        "constrained_columns", []
    )
    target_is_pk = target_pk and target_column in target_pk.get(
        "constrained_columns", []
    )
