...
class EnhancedNeo4jService:
    ...
    async def store_qa_pair_with_context(
        self, qa_pair: QAPairWithContext, schema_context: Dict[str, Any]
    ):
        """存储问答对及其完整上下文信息"""
        if not self._initialized:
            await self.initialize()
    ...
    async def structural_search(
        self, schema_context: Dict[str, Any], connection_id: int, top_k: int = 20
    ...
    def _build_qa_pair_from_record(
        self, qa_data, used_tables=None
    ) -> QAPairWithContext:
        """从Neo4j记录构建QAPair对象"""
...
class HybridRetrievalEngine:
    ...
    async def hybrid_retrieve(
        ...
        try:
            ...
            if settings.PARALLEL_RETRIEVAL:
                ...
                structural_results = (
                    structural_results
                    if not isinstance(structural_results, Exception)
                    else []
                )
                pattern_results = (
                    pattern_results
                    if not isinstance(pattern_results, Exception)
                    else []
                )
            else:
                # 串行执行
                ...
                structural_results = await self._structural_search(
                    schema_context, connection_id
                )
    ...
    async def _semantic_search(
        self, query: str, connection_id: int
    ) -> List[RetrievalResult]:
        """语义检索"""
        try:
            # 向量化查询
    ...
    async def _structural_search(
        self, schema_context: Dict[str, Any], connection_id: int
    ) -> List[RetrievalResult]:
        """结构检索"""
        try:
    ...
    async def store_qa_pair(
        ...
        try:
            ...
            if not qa_pair.embedding_vector:
                qa_pair.embedding_vector = await self.vector_service.embed_question(
                    qa_pair.question
                )
