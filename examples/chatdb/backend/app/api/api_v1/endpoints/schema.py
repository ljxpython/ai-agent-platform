...
def discover_connection_schema(
    *,
    ...
    try:
            ...
            if existing_table:
                ...
                for column_info in table_info["columns"]:
                    ...
                    existing_column = crud.schema_column.get_by_name_and_table(
                        db=db, column_name=column_name, table_id=existing_table.id
                    )
                    if existing_column:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error discovering schema: {str(e)}"
        )


@router.get(
    "/{connection_id}/metadata",
    response_model=List[schemas.SchemaTableWithRelationships],
)
    ...
    *,
    ...
    try:
        ...
        for table in tables:
            ...
            source_relationships = crud.schema_relationship.get_by_source_table(
                db=db, source_table_id=table.id
            )
            target_relationships = crud.schema_relationship.get_by_target_table(
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving schema metadata: {str(e)}"
        )
    ...
    try:
        ...
        for table_data in tables_data:
            table_obj = crud.schema_table.get_by_name_and_connection(
                db=db, table_name=table_data["table_name"], connection_id=connection_id
            )
            ...
            if table_obj:
                ...
                table_obj = crud.schema_table.update(
                    db=db, db_obj=table_obj, obj_in=table_update
                )
            else:
                # Create new table
            ...
            for column_data in table_data.get("columns", []):
                column_obj = crud.schema_column.get_by_name_and_table(
                    db=db, column_name=column_data["column_name"], table_id=table_obj.id
                )
                ...
                if column_obj:
                    ...
                    crud.schema_column.update(
                        db=db, db_obj=column_obj, obj_in=column_update
                    )
                else:
                    # Create new column
        ...
        existing_relationships = crud.schema_relationship.get_by_connection(
        ...
        for rel_data in relationships_data:
            ...
            source_table = crud.schema_table.get_by_name_and_connection(
                db=db, table_name=rel_data["source_table"], connection_id=connection_id
            )
            target_table = crud.schema_table.get_by_name_and_connection(
                db=db, table_name=rel_data["target_table"], connection_id=connection_id
            )
            ...
            source_column = crud.schema_column.get_by_name_and_table(
                db=db, column_name=rel_data["source_column"], table_id=source_table.id
            )
            target_column = crud.schema_column.get_by_name_and_table(
                db=db, column_name=rel_data["target_column"], table_id=target_table.id
            )
            ...
            if rel_obj:
                ...
                crud.schema_relationship.update(
                    db=db, db_obj=rel_obj, obj_in=rel_update
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error publishing schema: {str(e)}"
        )
...
def get_saved_schema(
    *,
    ...
    try:
        ...
        return {"tables": tables_data, "relationships": relationships_data}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error retrieving saved schema: {str(e)}"
        )
    ...
    *,
    ...
    try:
        ...
        if result:
            return {
                "status": "success",
                "message": "Schema synced to Neo4j successfully",
            }
        else:
...
def discover_and_sync(
    *,
    ...
    try:
        ...
        tables_data, relationships_data = save_discovered_schema(
            db, connection_id, schema_info
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error discovering and syncing schema: {str(e)}"
        )
