...
async def stream_response(
    ...
    # 记录请求参数
    logger.info(
        f"SSE请求参数: query={query}, connection_id={connection_id}, session_id={session_id}, user_feedback_enabled={user_feedback_enabled}"
    )
    ...
    if not session_id:
        ...
        if session_id not in active_sessions:
            ...
            if query:
                ...
                raise HTTPException(
                    status_code=404, detail=f"会话 {session_id} 不存在或已过期"
                )
        else:
    ...
    if create_new_session:
        ...
        try:
            ...
            if direct_process:
                ...
                asyncio.create_task(
                    process_query_task(
                        query, session_id, connection_id, user_feedback_enabled, db
                    )
                )
                ...
                # 启动处理任务
                logger.info(
                    f"正在启动异步查询处理任务: {session_id}, 查询: {query}, 连接ID: {connection_id}"
                )
...
async def event_generator(session_id: str, request: Request):
    ...
    if not message_queue:
        error_data = json.dumps({"error": "会话队列不存在"})
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                logger.info(
                    f"发送事件: id={message_id}, type={event_type}, region={message.get('region', 'unknown')}, content={content_preview}"
                )
        ...
        error_data = json.dumps({"error": f"生成事件流时出错: {str(e)}"})
        yield f"event: error\nid: error-{message_id}\ndata: {error_data}\n\n"

    # 发送关闭事件
    close_data = json.dumps({"message": "流已关闭"})
...
async def process_query_task(
    ...
    logger.info(
        f"===== 开始执行 process_query_task: 会话ID={session_id}, 查询={query}, 连接ID={connection_id}, 用户反馈={user_feedback_enabled} ====="
    )
    ...
    try:
        ...
        async def message_callback(
            ctx: ClosureContext, message: ResponseMessage, message_ctx: MessageContext
        ) -> None:
            try:
                # 转换为字典
                msg_dict = (
                ...
                region = "process"  # 默认
                if (
                    message.source == AGENT_NAMES.get("query_analyzer")
                    or message.source == AGENT_NAMES.get("schema_retriever")
                    or message.source == "查询分析智能体"
                    or message.source == "表结构检索智能体"
                ):
                    region = "analysis"
                elif (
                    message.source == AGENT_NAMES.get("sql_generator")
                    or message.source == "SQL生成智能体"
                    or message.source == "混合SQL生成智能体"
                ):
                    region = "sql"
                elif (
                    message.source == AGENT_NAMES.get("sql_explainer")
                    or message.source == "SQL解释智能体"
                ):
                    region = "explanation"
                elif (
                    message.source == AGENT_NAMES.get("sql_executor")
                    or message.source == "SQL执行智能体"
                ):
                    region = "data"
                elif (
                    message.source == AGENT_NAMES.get("visualization_recommender")
                    or message.source == "可视化推荐智能体"
                ):
                ...
                if message.result and message.is_final:
                    logger.info(
                        f"处理包含结果数据的最终消息: {list(message.result.keys())}"
                    )
                    ...
                    if "results" in result_data:
                        ...
                        await message_queue.put(
                            {
                                ...
                                "source": message.source,
                                "content": json.dumps(
                                    result_data["results"], ensure_ascii=False
                                ),
                                "region": "data",
                                "is_final": True,
                                "timestamp": datetime.now().isoformat(),
                        ...
                        logger.info(
                            f"发送数据结果到队列: {len(result_data['results'])} 条记录"
                        )
                    ...
                    if "explanation" in result_data:
                        ...
                        logger.info(
                            f"发送解释结果到队列: {result_data['explanation'][:50]}..."
                        )

                    if (
                        "visualization_type" in result_data
                        or "visualization_config" in result_data
                    ):
                        # 可视化结果
                        viz_data = {
        ...
        # 处理查询
        logger.info(
            f"process_query_task: 开始处理查询: {query}, 会话ID: {session_id}, 连接ID: {connection_id}"
        )
        try:
            result = await orchestrator.process_query(
                query, collector, connection_id, user_feedback_enabled
            )
            ...
            if result:
                ...
                try:
                        ...
                        await message_queue.put(
                            {
                                ...
                                "source": "系统",
                                "content": json.dumps(
                                    result_dict["results"], ensure_ascii=False
                                ),
                                "region": "data",
                                "is_final": True,
                                "timestamp": datetime.now().isoformat(),
                    ...
                    # 发送可视化结果
                    if (
                        "visualization_type" in result_dict
                        and result_dict["visualization_type"]
                    ) or (
                        "visualization_config" in result_dict
                        and result_dict["visualization_config"]
                    ):
                        viz_data = {
                        ...
                        logger.info(
                            f"process_query_task: 可视化结果已发送: {session_id}"
                        )
            ...
            active_sessions[session_id]["status"] = "completed"
            logger.info(
                f"process_query_task: 会话状态已更新为'completed': {session_id}"
            )
...
async def send_feedback(session_id: str, feedback: Dict[str, Any]):
    """
    发送用户反馈到指定会话
    """
    # 验证会话是否存在
    ...
    if not feedback_queue:
        raise HTTPException(
            status_code=500, detail=f"会话 {session_id} 的反馈队列不存在"
        )
    ...
    return JSONResponse(
        {"status": "success", "message": "反馈已发送", "session_id": session_id}
    )
...
async def delete_session(session_id: str):
    ...
    return JSONResponse({"status": "success", "message": f"会话 {session_id} 已删除"})


@router.post("/save-history")
async def save_session_history(
    history_request: SaveChatHistoryRequest, db: Session = Depends(deps.get_db)
):
    """
    保存会话历史到数据库
    """
