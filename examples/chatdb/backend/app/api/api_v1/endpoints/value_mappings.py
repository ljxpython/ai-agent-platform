...
def read_value_mappings(
    ...
    """
    if column_id:
        mappings = crud.value_mapping.get_by_column(
            db=db, column_id=column_id, skip=skip, limit=limit
        )
    else:
...
def create_value_mapping(
    *,
    ...
    existing = crud.value_mapping.get_by_column_and_term(
        db=db, column_id=mapping_in.column_id, nl_term=mapping_in.nl_term
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="Mapping already exists for this term"
        )
...
def update_value_mapping(
    *,
    ...
    if mapping_in.nl_term and mapping_in.nl_term != mapping.nl_term:
        existing = crud.value_mapping.get_by_column_and_term(
            db=db, column_id=mapping.column_id, nl_term=mapping_in.nl_term
        )
        if existing:
            raise HTTPException(
                status_code=400, detail="Mapping already exists for this term"
