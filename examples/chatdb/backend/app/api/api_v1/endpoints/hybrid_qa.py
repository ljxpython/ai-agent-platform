...
async def create_qa_pair(qa_create: QAPairCreate, db: Session = Depends(deps.get_db)):
    """创建新的问答对"""
    try:
        ...
        mentioned_entities = (
            qa_create.mentioned_entities
            or extract_entities_from_question(qa_create.question)
        )
        ...
        return {"status": "success", "qa_id": qa_pair.id, "message": "问答对创建成功"}
...
async def search_similar_qa_pairs(
    search_request: SearchRequest, db: Session = Depends(deps.get_db)
):
    """搜索相似的问答对"""
    try:
        engine = await get_hybrid_engine()
...
async def submit_feedback(
    feedback: FeedbackRequest, db: Session = Depends(deps.get_db)
):
    """提交用户反馈"""
    try:
        # 这里可以添加反馈处理逻辑
        ...
        logger.info(
            f"收到反馈 - QA ID: {feedback.qa_id}, 满意度: {feedback.user_satisfaction}"
        )
...
async def batch_create_qa_pairs(
    qa_pairs: List[QAPairCreate], db: Session = Depends(deps.get_db)
):
    """批量创建问答对"""
    try:
        engine = await get_hybrid_engine()
        ...
        for i, qa_create in enumerate(qa_pairs):
            try:
                # 自动提取表名和实体
                used_tables = qa_create.used_tables or extract_tables_from_sql(
                    qa_create.sql
                )
                mentioned_entities = (
                    qa_create.mentioned_entities
...
async def delete_qa_pair(qa_id: str, db: Session = Depends(deps.get_db)):
    """删除问答对"""
    try:
        # 这里可以添加删除逻辑
        ...
        return {"status": "success", "message": f"问答对 {qa_id} 删除成功"}
