    ...
    try:
        ...
        with driver.session() as session:
                ...
                result["edges"].append(
                    {
                        ...
                        "type": "hasColumn",
                        "data": {"relationshipType": "HAS_COLUMN"},
                    }
                )
            ...
            relationship_records = session.run(
                relationship_query, connection_id=connection_id
            )
        ...
        driver.close()
        print(
            f"Returning result with {len(result['nodes'])} nodes and {len(result['edges'])} edges"
        )
        return result
