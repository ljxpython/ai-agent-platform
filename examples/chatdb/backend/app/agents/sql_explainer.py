...
from app.schemas.text2sql import SqlExplanationMessage, SqlMessage
...
class SqlExplainerAgent(BaseAgent):
    ...
    def _prepare_prompt(
        self, query: str, sql: str, schema_context: Dict[str, Any] = None
    ) -> str:
        """准备提示模板
        ...
        # 格式化表结构信息
        db_schema_str = (
            self.format_schema_context(schema_context)
            if schema_context
            else self.db_schema
        )
        ...
        prompt = self._prompt_template.format(
            db_type=self.db_type, db_schema=db_schema_str, query=query, sql=sql
        )
        return prompt
    ...
    async def handle_message(self, message: SqlMessage, ctx: MessageContext) -> None:
        ...
        try:
            ...
            await self.publish_message(
                explanation_message,
                topic_id=TopicId(
                    type=TopicTypes.SQL_EXECUTOR.value, source=self.id.key
                ),
            )
