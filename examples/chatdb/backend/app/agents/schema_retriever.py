...
from app.services.text2sql_service import get_value_mappings, retrieve_relevant_schema
...
class SchemaRetrieverAgent(BaseAgent):
    ...
    async def retrieve_schema(
        self, connection_id: int, query: str
        ...
        try:
            ...
            try:
                ...
                schema_context = await retrieve_relevant_schema(
                    db=db, connection_id=connection_id, query=query
                )
    ...
    async def handle_message(self, message: QueryMessage, ctx: MessageContext) -> None:
        ...
        try:
            ...
            if message.connection_id:
                ...
                # 从图数据库获取相关表结构信息和值映射
                schema_context, mappings_str = await self.retrieve_schema(
                    connection_id, message.query
                )

                # 输出找到的相关表信息
                if schema_context and schema_context.get("tables"):
                    tables_info = "\n".join(
                        [f"- {table['name']}" for table in schema_context["tables"]]
                    )
                    await self.send_response(f"已找到以下相关表:\n{tables_info} \n\n")
                else:
                    await self.send_response(
                        "未找到相关表结构，将使用默认数据库结构继续分析\n\n"
                    )
            else:
                await self.send_response(
                    "未提供数据库连接ID，将使用默认数据库结构继续分析\n\n"
                )
            ...
            await self.publish_message(
                ...
                topic_id=TopicId(
                    type=TopicTypes.QUERY_ANALYZER.value, source=self.id.key
                ),
            )
        except Exception as e:
