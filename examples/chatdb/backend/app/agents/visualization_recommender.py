...
class VisualizationRecommenderAgent(BaseAgent):
    ...
    def _parse_visualization_config(
        self, content: str, fallback_results: list
    ) -> Dict[str, Any]:
        """解析可视化配置
        ...
        try:
            ...
            return {
                ...
                "config": {
                    "title": "数据可视化",
                    "xAxis": (
                        list(fallback_results[0].keys())[0] if fallback_results else "x"
                    ),
                    "yAxis": (
                        list(fallback_results[0].keys())[1]
                        if fallback_results and len(fallback_results[0].keys()) > 1
                        else "y"
                    ),
                },
            }
    ...
    async def handle_message(
        self, message: SqlResultMessage, ctx: MessageContext
    ) -> None:
        """处理接收到的消息，推荐可视化方式"""
        try:
            # 检查是否有错误
            if hasattr(message, "error") and message.error:
                await self.send_response(
                    f"由于SQL执行错误，无法提供可视化建议: {message.error}"
                )
            ...
            # 处理JSON输出
            visualization = self._parse_visualization_config(
                visualization_json, message.results
            )
