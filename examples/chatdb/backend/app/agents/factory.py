...
class AgentFactory:
    ...
    async def _register_schema_retriever(
        ...
        await SchemaRetrieverAgent.register(
            ...
            lambda: SchemaRetrieverAgent(
                model_client_instance=model_client, db_type=self.db_type
            ),
        )

    async def _register_query_analyzer(
        self,
        runtime: SingleThreadedAgentRuntime,
        collector: StreamResponseCollector,
        user_feedback_enabled: bool = False,
    ...
    async def _register_sql_generator(
        self, runtime: SingleThreadedAgentRuntime
    ) -> None:
        """注册SQL生成智能体（根据配置选择）"""
    ...
    async def _register_sql_explainer(
        self, runtime: SingleThreadedAgentRuntime
    ) -> None:
        """注册SQL解释智能体"""
    ...
    async def _register_visualization_recommender(
        self, runtime: SingleThreadedAgentRuntime
    ) -> None:
        """注册可视化推荐智能体"""
    ...
    async def _register_stream_collector(
        self, runtime: SingleThreadedAgentRuntime, collector: StreamResponseCollector
    ) -> None:
        """注册消息收集器"""
