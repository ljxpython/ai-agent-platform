...
class SqlExecutorAgent(BaseAgent):
    ...
    def __init__(
        ...
        db_type=None,
        db_access: Optional[DBAccess] = None,
    ):
        """初始化SQL执行智能体
    ...
    async def _execute_sql(
        self, sql: str, connection_id: Optional[int] = None
    ...
    async def handle_message(
        self, message: SqlExplanationMessage, ctx: MessageContext
    ) -> None:
        """处理接收到的消息，执行SQL查询"""
        try:
            ...
            if error_msg:
                ...
                await self.publish_message(
                    ...
                    topic_id=TopicId(
                        type=TopicTypes.VISUALIZATION_RECOMMENDER.value,
                        source=self.id.key,
                    ),
                )
            else:
