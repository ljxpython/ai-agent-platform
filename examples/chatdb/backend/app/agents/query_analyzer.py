...
from app.schemas.text2sql import AnalysisMessage, SchemaContextMessage
...
"""
    ...
    def _prepare_task(
        self, query: str, schema_context: Dict[str, Any], mappings_str: str = ""
    ) -> str:
        """准备具体的分析任务
    ...
    async def handle_message(
        self, message: SchemaContextMessage, ctx: MessageContext
    ) -> None:
        ...
        try:
            ...
            if self.input_func:
                ...
                stream = team.run_stream(
                    task=self._prepare_task(query, schema_context, mappings_str)
                )
            ...
            await self.publish_message(
                analysis_message,
                topic_id=TopicId(
                    type=TopicTypes.SQL_GENERATOR.value, source=self.id.key
                ),
            )
