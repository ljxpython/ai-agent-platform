...
from autogen_core import ClosureContext, MessageContext, RoutedAgent, TopicId
...
class BaseAgent(RoutedAgent, ABC):
    ...
    def __init__(
        self, agent_id: str, agent_name: str, model_client_instance=None, db_type=None
    ):
        """初始化基础智能体
    ...
    async def send_response(
        self, content: str, is_final: bool = False, result: Dict[str, Any] = None
    ) -> None:
        """发送响应消息到流输出主题
    ...
    def format_schema_context(self, schema_context: Dict[str, Any]) -> str:
        ...
        for table in schema_context["tables"]:
            ...
            for column in columns:
                ...
                db_schema_str += (
                    f"    [{column['name']}] {column['type']} {pk_flag} {fk_flag},\n"
                )
    ...
    def format_schema_as_markdown(self, schema_context: Dict[str, Any]) -> str:
        ...
        for table in schema_context["tables"]:
            table_name = table["name"]
            table_desc = (
                f" - {table['description']}" if table.get("description") else ""
            )
    ...
    def rebuild_memory_from_content(
        self, memory_content: List[Dict[str, Any]]
    ) -> ListMemory:
...
class StreamResponseCollector:
    ...
    def __init__(self):
        ...
        self.callback: Optional[
            Callable[[ClosureContext, ResponseMessage, MessageContext], Awaitable[None]]
        ] = None
    ...
    async def buffer_message(
        ...
        if is_final or result:
            ...
            if source in self.message_buffers and self.message_buffers[source]:
                await self.callback(
                    ctx,
                    ResponseMessage(
                        source=source, content=self.message_buffers[source]
                    ),
                    None,
                )
            ...
            await self.callback(
                ctx,
                ResponseMessage(
                    source=source, content=content, is_final=is_final, result=result
                ),
                None,
            )
            return
        ...
        # 检查是否需要刷新缓冲区
        if (
            current_time - self.last_flush_time.get(source, 0)
            >= self.buffer_flush_interval
        ):
            await self.callback(
                ctx,
                ResponseMessage(source=source, content=self.message_buffers[source]),
                None,
            )
            self.message_buffers[source] = ""
            self.last_flush_time[source] = current_time
    ...
    async def flush_all_buffers(self, ctx: ClosureContext = None) -> None:
        ...
        for source, content in self.message_buffers.items():
            if content.strip():  # 只发送非空内容
                await self.callback(
                    ctx, ResponseMessage(source=source, content=content), None
                )
