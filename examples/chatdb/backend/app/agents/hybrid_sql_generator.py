...
from .types import AGENT_NAMES, AgentTypes, TopicTypes
...
class HybridSqlGeneratorAgent(BaseAgent):
    ...
    def _prepare_prompt(
        self, schema_context: Dict[str, Any] = None, mappings_str: str = ""
    ) -> str:
        """准备提示模板
    ...
    async def handle_analysis_message(
        self, message: AnalysisMessage, ctx: MessageContext
    ) -> None:
        ...
        try:
            ...
            if self._initialized and settings.HYBRID_RETRIEVAL_ENABLED:
                try:
                    ...
                    if similar_examples:
                        ...
                        system_message = self._prepare_prompt(
                            schema_context, mappings_str
                        )

                except Exception as e:
                    await self.send_response(
                        f"检索示例时出错，使用标准模式: {str(e)} \n\n"
                    )
            ...
            if (
                self._initialized
                and settings.AUTO_LEARNING_ENABLED
                and self._is_valid_sql(sql_content)
            ):
                await self._learn_qa_pair(
                    query, sql_content, schema_context, connection_id
                )
            ...
            sql_message = SqlMessage(
                query=query, sql=sql_content, connection_id=connection_id
            )

            await self.publish_message(
                sql_message,
                topic_id=TopicId(
                    type=TopicTypes.SQL_EXPLAINER.value, source=self.id.key
                ),
            )
    ...
    def _build_enhanced_prompt_with_examples(
        self, query: str, schema_context: Dict, examples: List, mappings_str: str
    ) -> str:
        """构建包含示例的增强提示"""
...
"""
    ...
    async def _learn_qa_pair(
        self, question: str, sql: str, schema_context: Dict, connection_id: int
    ):
        """学习新的问答对"""
        try:
    ...
    def _classify_query_type(self, question: str) -> str:
        ...
        if any(
            ...
            for word in ["count", "sum", "avg", "max", "min", "统计", "计算", "总数"]
        ):
            return "AGGREGATE"
