...
class CRUDDBConnection(CRUDBase[DBConnection, DBConnectionCreate, DBConnectionUpdate]):
    ...
    def update(
        ...
        *,
        db_obj: DBConnection,
        obj_in: Union[DBConnectionUpdate, Dict[str, Any]],
    ) -> DBConnection:
    ...
    def remove(self, db: Session, *, id: int) -> DBConnection:
        ...
        try:
            ...
            db.execute(
                delete(SchemaRelationship).where(SchemaRelationship.connection_id == id)
            )
                ...
                if column_ids:
                    ...
                    db.execute(
                        delete(ValueMapping).where(
                            ValueMapping.column_id.in_(column_ids)
                        )
                    )

                # Step 3: Delete all columns associated with these tables
                db.execute(
                    delete(SchemaColumn).where(SchemaColumn.table_id.in_(table_ids))
                )
