...
from app.models.chat_history import ChatHistorySnapshot, ChatMessage, ChatSession
...
class CRUDChatSession(CRUDBase[ChatSession, ChatSessionCreate, ChatSessionUpdate]):
    def get_by_user_sessions(
        ...
        *,
        ...
        return (
            query.order_by(desc(self.model.updated_at)).offset(skip).limit(limit).all()
        )
    ...
    def get_with_messages(
        ...
        return (
            db.query(self.model)
            .filter(and_(self.model.id == session_id, self.model.is_active == True))
            .first()
        )
...
class CRUDChatMessage(CRUDBase[ChatMessage, ChatMessageCreate, ChatMessageUpdate]):
    ...
    def create_batch(
        self, db: Session, *, messages: List[ChatMessageCreate]
    ) -> List[ChatMessage]:
        """批量创建消息"""
        db_messages = []
