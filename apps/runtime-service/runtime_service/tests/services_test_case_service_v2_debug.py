...
from runtime_service.services.test_case_service_v2.graph import (  # noqa: E402
    TEST_CASE_DEFAULTS,
    graph,
)
from runtime_service.services.test_case_service_v2.prompts import SYSTEM_PROMPT  # noqa: E402
...
def _summarize_update(event: Any) -> dict[str, Any]:
    ...
    for key, value in event.items():
        ...
        if key == "TestCaseDocumentPersistenceMiddleware.before_model" and isinstance(
            value, dict
        ):
            attachments = value.get(MULTIMODAL_ATTACHMENTS_KEY)
            summary[key] = {
    ...
    *,
    ...
    return {
        ...
        "knowledge_tool_calls": [
            name for name in tool_calls if name in KNOWLEDGE_TOOL_NAMES
        ],
        ...
        "document_persistence_updates": [
            ...
            if isinstance(item, dict)
            and isinstance(
                item.get("TestCaseDocumentPersistenceMiddleware.before_model"), dict
            )
        ],
        "updates": update_summaries,
        "output_text": "".join(stream_chunks),
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Debug test_case_service_v2 with a real PDF and a real model."
    )
    ...
    parser.add_argument(
        "--project-id",
        default=None,
        help="可选：显式 project_id。若需要知识库或持久化，必须提供 UUID。",
    )
    parser.add_argument(
        "--enable-knowledge",
        action="store_true",
        help="启用 LightRAG project-scoped MCP。",
    )
    parser.add_argument(
        "--knowledge-mcp-url",
        default=None,
        help="可选：覆盖 v2 私有知识库 MCP SSE 地址。",
    )
    parser.add_argument(
        "--knowledge-timeout",
        type=int,
    ...
    parser.add_argument(
        "--enable-persistence",
        action="store_true",
        help="启用正式落库链路；未开启时默认关闭持久化，便于纯多模态调试。",
    )
    return parser
...
async def _main_async(args: argparse.Namespace) -> int:
    ...
    config: RunnableConfig = {
        "configurable": {
            "thread_id": str(uuid4()),
            "test_case_v2_knowledge_mcp_enabled": bool(args.enable_knowledge),
            "test_case_v2_persistence_enabled": bool(args.enable_persistence),
        }
    }
    if args.parser_model_id:
        config["configurable"]["test_case_v2_multimodal_parser_model_id"] = (
            args.parser_model_id
        )
    if args.knowledge_mcp_url:
        config["configurable"]["test_case_v2_knowledge_mcp_url"] = args.knowledge_mcp_url
    if args.knowledge_timeout is not None:
        config["configurable"]["test_case_v2_knowledge_timeout_seconds"] = (
            args.knowledge_timeout
        )
    if args.knowledge_sse_read_timeout is not None:
        config["configurable"]["test_case_v2_knowledge_sse_read_timeout_seconds"] = (
    ...
    _print_section(
        "Input",
        {
            ...
            **model_preview,
            ...
            "persistence_enabled": service_config.persistence_enabled,
        },
    )
    _print_section(
        "Frontend Block",
