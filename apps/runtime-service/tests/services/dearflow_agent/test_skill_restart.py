"""Real checkpoints, process restart and read-only container skill mounts."""
import os
...
from .test_p6_governance import dsn, package
...
async def run():
    ...
    command = "(if test -f /skills/custom/test-skill/SKILL.md; then cat /skills/custom/test-skill/SKILL.md; else echo ABSENT; fi) > /workspace/work/" + stage + ".txt"
...
asyncio.run(run())
'''
    env = {**os.environ, "DATABASE_URI": dsn, "RUNTIME_WORKSPACE_ROOT": str(tmp_path),
           "RUNTIME_DEAR_GOVERNANCE_ENABLED": "1", "SKILL_TEST_NEXT": "VALUE_B" if change == "update" else "ABSENT",
    ...
    for stage in ("pause", "resume", "next"):
        result = subprocess.run([sys.executable, "-c", script], env={**env, "SKILL_TEST_STAGE": stage},
                                capture_output=True, text=True, timeout=180)
        assert result.returncode == 0, result.stderr[-10000:]
        if stage == "pause":
            if change == "update":
                store.update(scope, "test-skill", package("VALUE_B"), expected_revision=a["revision"])
            elif change == "disable":
                store.set_enabled(scope, "test-skill", enabled=False, expected_revision=a["revision"])
            else:
...
def test_child_file_tools_share_resumed_snapshot(dsn, tmp_path, monkeypatch):
    ...
    from runtime_service.services.dearflow_agent.workspace.backend import DearWorkspaceBackend, build_backend
    ...
    def graph(resuming):
        ...
        return create_deep_agent(
            model=BindableFakeMessagesChatModel(responses=[AIMessage(content="done")] if resuming else
                [call("task", {"description": "Read skill", "subagent_type": "reader"})]),
            backend=backend,
            middleware=[ExecutionSkillsMiddleware(workspace, backend, custom_enabled=True)],
            subagents=[{"name": "reader", "description": "Read a skill", "system_prompt": "Read only",
                        "model": ChildModel(responses=[call("read_file", {"file_path": "/skills/custom/test-skill/SKILL.md"}), AIMessage(content="child done")])}],
            interrupt_on={"task": True}, checkpointer=saver)
    async def run():
        cfg = config()
        result = await graph(False).ainvoke({"messages": [("user", "delegate")]}, cfg)
        assert result["__interrupt__"]
        store.update(scope, "test-skill", package("CHILD_VALUE_B"), expected_revision=a["revision"])
        await graph(True).ainvoke(Command(resume={"decisions": [{"type": "approve"}]}), cfg)
