...
def _token(
    *, request_id: str | None = None, platform_trace_id: str | None = None
) -> str:
    now = int(time.time())
    claims = {
...
def test_platform_auth_returns_runtime_facts_without_token(
...
def test_platform_auth_preserves_signed_trace_correlation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PLATFORM_RUNTIME_DELEGATION_SECRET", SECRET)
    monkeypatch.setenv("PLATFORM_RUNTIME_DELEGATION_ISSUER", "runtime-test")
...
def test_platform_auth_requires_audience_configuration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PLATFORM_RUNTIME_DELEGATION_SECRET", SECRET)
    monkeypatch.setenv("PLATFORM_RUNTIME_DELEGATION_ISSUER", "runtime-test")
    ...
    with pytest.raises(Auth.exceptions.HTTPException) as error:
        asyncio.run(
            deny_image_scope_on_server_resources(
                SimpleNamespace(user={"runtime_scope": {"operation": operation}}), {}
            )
        )
...
def test_custom_scope_is_rejected_for_attribute_user():
    ...
    with pytest.raises(Auth.exceptions.HTTPException) as error:
        asyncio.run(
            deny_image_scope_on_server_resources(SimpleNamespace(user=user), {})
        )
    assert error.value.status_code == 403


def test_thread_auth_rechecks_signed_platform_acl(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from types import SimpleNamespace
    ...
    monkeypatch.setenv(
        "PLATFORM_THREAD_AUTHORIZATION_URL",
        "http://platform.test/api/runtime/internal/thread-authorization",
    )
    monkeypatch.setenv("PLATFORM_RUNTIME_DELEGATION_SECRET", SECRET)
    response = SimpleNamespace(
        raise_for_status=lambda: None, json=lambda: {"allowed_thread_ids": ["thread-1"]}
    )
...
def test_thread_create_accepts_graphharbor_uuid_target(
    ...
    response = SimpleNamespace(
        raise_for_status=lambda: None,
        json=lambda: {"allowed_thread_ids": [str(thread_id)]},
    )
...
def test_thread_reconcile_only_reads_its_bound_pending_thread(
    ...
    with pytest.raises(Auth.exceptions.HTTPException) as wrong_target:
        asyncio.run(
            platform.deny_image_scope_on_server_resources(
                ctx, {"thread_id": "thread-2"}
            )
        )
    assert wrong_target.value.status_code == 403
    with pytest.raises(Auth.exceptions.HTTPException) as wrong_action:
        asyncio.run(
            platform.deny_image_scope_on_server_resources(
                SimpleNamespace(**{**ctx.__dict__, "action": "update"}),
                {"thread_id": "thread-1"},
            )
        )
...
def test_only_the_delegated_assistant_is_readable() -> None:
    ...
    asyncio.run(
        deny_image_scope_on_server_resources(ctx, {"assistant_id": "assistant-a"})
    )
    asyncio.run(
        deny_image_scope_on_server_resources(
            ctx, {"assistant_id": uuid5(NAMESPACE_URL, "assistant-a")}
        )
    )
    with pytest.raises(Auth.exceptions.HTTPException) as error:
        asyncio.run(
...
def test_read_delegation_cannot_mutate_threads(action: str) -> None:
    ...
    with pytest.raises(Auth.exceptions.HTTPException) as error:
        asyncio.run(
            deny_image_scope_on_server_resources(ctx, {"thread_id": "thread-1"})
        )
    assert error.value.status_code == 403


@pytest.mark.parametrize(
    ("operation", "action", "run_id"),
    [
        ("run-create", "create_run", None),
        ("thread-edit", "update", None),
        ("thread-delete", "delete", None),
        ("run-cancel", "update", "run-1"),
        ("run-delete", "delete", "run-1"),
    ],
)
def test_scoped_write_delegations_allow_only_their_bound_action(
    operation, action, run_id, monkeypatch
):
    from types import SimpleNamespace
    from unittest.mock import AsyncMock
    from runtime_service.auth import platform
    ...
    monkeypatch.setattr(platform.httpx, "AsyncClient", Client)
    scope = {"operation": operation, "thread_id": "thread-1", "assistant_id": "agent-a"}
    ctx = SimpleNamespace(
        user={"identity": "user-a", "project_id": "project-a", "runtime_scope": scope},
        resource="threads",
        action=action,
    )
    value = {"thread_id": "thread-1", "assistant_id": "agent-a"}
    if run_id:
        value["run_id"] = run_id
    asyncio.run(platform.deny_image_scope_on_server_resources(ctx, value))

    with pytest.raises(Auth.exceptions.HTTPException) as wrong_thread:
        asyncio.run(
            platform.deny_image_scope_on_server_resources(
                ctx, {**value, "thread_id": "thread-2"}
            )
        )
...
def test_thread_auth_fails_closed_for_unbounded_or_denied_targets(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from types import SimpleNamespace
    from unittest.mock import AsyncMock
    ...
    response = SimpleNamespace(
        raise_for_status=lambda: None, json=lambda: {"allowed_thread_ids": []}
    )
