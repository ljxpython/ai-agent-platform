...
- Execute read-only SELECT queries against uploaded data
...
- **File location**: Path(s) to uploaded Excel/CSV files under `/workspace/uploads/`
...
python /skills/data-analysis/scripts/analyze.py \
  ...
  --output-file /workspace/work/filtered-results.csv
```
...
```bash
python /skills/data-analysis/scripts/analyze.py \
  --files /workspace/uploads/sales_2024.xlsx \
  --action inspect
```
...
python /skills/data-analysis/scripts/analyze.py \
  ...
  --output-file /workspace/work/monthly-trends.csv
```
...
```bash
python /skills/data-analysis/scripts/analyze.py \
  --files /workspace/uploads/orders.csv /workspace/uploads/customers.xlsx \
  --action query \
...
- For large results, export to file and share via `present_artifacts` tool
...
Each invocation owns an in-memory database; no cache or disk spill. Input limit 20 MiB per file; Excel limit 100,000 cells per sheet. Use the schema output for actual table and column names.
...
- Inputs exceeding execution memory or row/cell limits fail explicitly; reduce the data before retrying.
- Column names with spaces are accessible using double quotes: `"Column Name"`

## Dear Agent execution contract
Use the approved `execute` tool to run the script in the fixed offline image. No pip, extensions, network or shared cache. Inspect first; table names are generated uniquely and mapping keys include file and sheet. SELECT only, at most 10,000 result rows; larger results fail explicitly. CSV/XLSX/XLS inputs remain read-only. Formulas are never evaluated; XLSX formulas are shown as text, XLS uses stored values. Outputs must be under /workspace/work/; publish CSV/JSON/Markdown with present_artifacts. CSV formula-like strings are escaped for spreadsheet safety.
