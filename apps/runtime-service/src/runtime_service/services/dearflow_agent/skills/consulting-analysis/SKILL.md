...
- Dear Agent integration: clarify missing decision-critical inputs with the official `request_information` tool. Keep facts, assumptions, calculations, and data gaps explicit. When chart-generation skills are unavailable, retain the visualization plan and mark charts as not generated; never invent chart paths. Publish each Markdown framework/report with `present_artifacts(file_path=...)` from `/workspace/work/`.
