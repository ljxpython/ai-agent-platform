...
Dear Agent integration: only generate downloadable Markdown, not email or subscriptions. After search_web, call fetch_page to verify the body of selected news sources before labeling them verified; search snippets alone are not body verification. Use source publication dates when verified; otherwise label the date unknown. Retrieval time is not publication time. Deduplicate links, exclude known out-of-window stories, and distinguish user-provided samples from verified online news. Publish each file with `present_artifacts(file_path=...)`.
...
For key stories, use `fetch_page` to read full articles and extract:
...
- Save the newsletter to `/workspace/work/newsletter-{topic}-{date}.md`
- Present the newsletter to the user using the `present_artifacts` tool
