...
For an uploaded source ZIP, use `parse_document(file_path=<actual upload path>)` to list entries and read text in memory. If truncated, call it again with `query=<file path substring>` to inspect each relevant file. Cite the returned archive path and member path. Do not install dependencies, execute project code, or extract files onto the host. The directory commands below illustrate analysis of an existing authorized workspace; they do not refer to a directory automatically created from a ZIP upload. Document unsupported binary content and missing files explicitly.
...
ls /workspace/uploads/project-dir/

# Read key files
read_file /workspace/uploads/project-dir/package.json
read_file /workspace/uploads/project-dir/pyproject.toml

# Search for public API surfaces
grep -r "export " /workspace/uploads/project-dir/src/
grep -r "def " /workspace/uploads/project-dir/src/ --include="*.py"
grep -r "func " /workspace/uploads/project-dir/ --include="*.go"
```
...
- Save documentation files to `/workspace/work/`
- For multi-file documentation, maintain the project directory structure
- Present generated files to the user using the `present_artifacts` tool
