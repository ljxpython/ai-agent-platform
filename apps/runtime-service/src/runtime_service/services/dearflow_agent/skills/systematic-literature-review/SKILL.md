...
- **Output location**: where to save the final report (default `/workspace/work/`).
...
Call the authorized `arxiv_search` tool. The bundled script is retained as upstream reference; its query construction and Atom parsing are reused by the service. Do not execute it or create another network client.

```text
arxiv_search(query="<topic>", max_results=20, category="cs.CL",
             sort_by="relevance", start_date="2024-01-01", end_date="2026-12-31")
```

Omit optional category/date arguments when unspecified. Examples below use the original CLI option notation: map `--max-results`, `--sort-by`, `--start-date`, `--end-date` to the corresponding underscore-named tool arguments; do not run a CLI command.
...
The tool returns JSON with a `papers` array. Each paper has: `id`, `title`, `authors`, `abstract`, `published`, `updated`, `categories`, `pdf_url`, `abs_url`, `read_scope`, `missing_fields`. The envelope includes query, searched_at, duplicates_removed, returned, total_results, truncated, and evidence. Cite the saved evidence path. IDs are deduplicated across versions; missing metadata remains missing. `read_scope=abstract_only` must appear in the methodology: abstracts are not full-text review.
...
**If the tool returns fewer papers than requested**, report the returned count and any truncation or duplicate removal. Do not infer the total from one returned page and do not pad the list.
...
The tool automatically saves immutable search evidence. Keep the returned paper metadata in context for Phase 3; do not duplicate it in another work file. Save the final report and, when requested, a separate `.bib` reference file in Phase 5.
...
**Concurrency limit: at most 3 subagents per turn.** Dear Agent limits concurrent task execution to 3; excess calls wait rather than being silently dropped. The per-thread task budget is 10, sufficient for 50 papers in batches of 5. Keep the round strategy below and do not spend this budget on unrelated delegation.
...
**Parsing subagent results**: use the official Deep Agents task ToolMessage content directly; there is no DeerFlow success/failure prefix. Check the tool status and actual JSON array, including input/output IDs and count. An error message is not paper metadata. Note failed or missing papers explicitly; never silently count them as reviewed.
...
Save the full report to `/workspace/work/slr-<topic-slug>-<YYYYMMDD>.md` where `<topic-slug>` is a lowercased hyphenated version of the topic (e.g. `transformer-attention`). Then call the `present_artifacts` tool with that path so the user can download it.
...
5. Phase 5: save to `slr-transformer-attention-20260409.md`, call `present_artifacts`.
...
- **Prerequisite: Ultra mode and authorized `task` access**. Phase 3 requires the official task tool. When it is unavailable, explain the missing capability rather than claiming parallel extraction.
...
- **Phase 3 requires Ultra and task authorization**. If unavailable, explain the limitation or offer a smaller manual review without claiming to have executed this workflow.
- **Subagent results are strings, not objects**. Parse the actual JSON content without assuming DeerFlow prefixes; validate paper IDs and counts before synthesizing.
