...
Read uploaded PDFs with `parse_document` and cite the actual upload path plus page number. Follow `read_range`, `truncated`, and OCR warnings; request additional page ranges when needed. Empty/scanned/missing pages are not reviewed text. A URL fetched as a web page is not proof that the entire PDF was read. Publish one Markdown review at a time with `present_artifacts(file_path=...)`.
...
Use `fetch_page` on key related papers or surveys to understand where this work fits.
...
- Save the review to `/workspace/work/review-{paper-topic}.md` when working in sandbox
- Present the review to the user using the `present_artifacts` tool
