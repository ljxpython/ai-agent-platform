...
Use the approved `generate_image` or `edit_image` tools, with a stable
`idempotency_key` per requested image. Read `get_media_task` after an unknown
outcome; never choose a new key just to retry a purchase. Keys and model
configuration belong to Runtime, not shell scripts. The tools return
`result.runtime_images` only after real image validation and storage.
Editing accepts one `image_path` and at most three `reference_images`,
all immutable uploaded/generated image references from this thread.
Current provider is the platform's OpenAI-compatible image connection;
size is 1024x1024, one image per call. Other upstream provider/size examples
below are design references, not supported executable commands.
Never run the upstream network script, read API keys, or install packages.
...
This skill generates high-quality images using structured prompts and approved Runtime tools. The workflow includes creating JSON-formatted prompts and calling image generation tools with optional reference images.

## Core Capabilities

- Create structured JSON prompts for AIGC image generation
- Support multiple reference images for style/composition guidance
- Generate images through approved Runtime tools
...
- You don't need to check the folder under `/workspace`

### Step 2: Create Structured Prompt

Generate a structured JSON file in `/workspace/work/` with naming pattern: `{descriptive-name}.json`

### Step 3: Execute Generation

Call the Python script:
Use the approved `generate_image` tool with the structured prompt rendered as text and a stable `idempotency_key`. For a reference image, use `edit_image(image_path=..., prompt=..., idempotency_key=...)`; never run a network script.
...
[!NOTE]
Image provider calls must use approved tools; the script is disabled.

## Character Generation Example

User request: "Create a Tokyo street style woman character in 1990s"

Create prompt file: `/workspace/work/asian-woman.json`
```json
{
  "characters": [{
    "gender": "female",
...
- Images are typically saved in `/workspace/work/`
...
Only the platform-managed OpenAI-compatible connection is enabled.
`IMAGE_25_KEY`, `IMAGE_25_URL`, and `IMAGE_25_MODEL` are operator settings,
not values to read or change with tools. The current request is one 1024x1024
image. Gemini and MiniMax integrations from upstream are not enabled.
Provider support for multi-reference edits requires deployment acceptance.
The tool validates PNG/JPEG/WebP content before returning a reference.
