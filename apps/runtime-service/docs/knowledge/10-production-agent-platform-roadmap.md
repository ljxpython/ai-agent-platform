...
`agent.py/get_agent()` 中显式装配普通 Tool，代码中的工具列表就是该 Agent 的最大能力
...
Skills 和 Subagents；可运行 Demo 统一按 28 号开发计划实施。
...
第一轮基准使用全新的 `reference_agent`，不依赖旧 `assistant`。`workflow_demo` 同期验证
StateGraph；Deep Agent、MCP 和 Backend 能力在 R4 通过独立 Demo 验证。
