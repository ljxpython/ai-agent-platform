...
> 当前端 chat 基座切到 `@langchain/vue/useStream` 后，`apps/platform-api-v2` 的 `runtime_gateway` 必须保持哪些协议边界不变？

结论：

- 可以切，而且 `platform-api-v2` 当前的 `/api/langgraph` 网关已经足够接入官方 `Client + useStream`。
...
1. `platform-web-vue` 升级 `@langchain/langgraph-sdk` 时，`platform-api-v2` 不得再输出旧版私有字段
...
`platform-api-v2` 对 chat 的标准支持方式已经冻结为：
