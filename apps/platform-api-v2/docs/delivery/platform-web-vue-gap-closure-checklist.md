# Platform Web Vue 对齐与收口清单

这份清单用于收口 `apps/platform-api-v2` 和 `apps/platform-web-vue` 之间的正式能力边界，并修正当前前端中不合理、重复或过渡期残留的实现。

目标不是继续“堆功能”，而是把当前已经成立的控制面和正式前端宿主收成一套可以长期演进的标准范式。

## 1. 执行目标

- 冻结 `platform-api-v2` 的正式 public 能力和 internal 能力边界
- 让 `apps/platform-web-vue` 与当前后端正式能力一一对齐
...
- `platform-api-v2` 已存在能力的边界冻结与前端消费对齐
- `platform-web-vue` 中与当前正式范式不一致的实现收口
