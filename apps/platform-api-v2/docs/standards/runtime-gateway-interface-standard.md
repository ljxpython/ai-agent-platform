# Platform API V2 Runtime Gateway / 管理接口标准

文档类型：`Current Standard`

这份文档定义 `platform-api-v2` 在 **runtime gateway / formal platform management interface** 上的当前标准。
...
- 变更 `platform-web-vue -> platform-api-v2` 的正式平台接口
- 变更 `platform-api-v2 -> runtime-service` 的 runtime gateway 暴露面
...
- 供 `platform-web-vue` 正式消费的管理接口字段

### 2.5 formal interface 变更必须走真实验证

验证顺序固定为：

1. `platform-api-v2` 本地契约 / 单测 / 集成证明
2. 最短相关真实链路
   - `platform-web-vue -> platform-api-v2`
   - 或 `platform-api-v2 -> runtime-service`
...
- `platform-api-v2 -> interaction-data-service` 一侧的结果域资源语义判断
