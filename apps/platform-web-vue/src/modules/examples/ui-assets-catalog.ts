...
function createTemplateItem(mode: TemplateMode, sourceKey: string, loader: SourceLoader): Sub2apiTemplateItem {
...
function buildGroups(mode: TemplateMode, loaders: Record<string, SourceLoader>) {
  const items = Object.entries(loaders)
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([sourceKey, loader]) => createTemplateItem(mode, sourceKey, loader))
...
export const sub2apiTemplateGroups = {
  pages: buildGroups('pages', pageLoaders),
  components: buildGroups('components', componentLoaders),
  engineering: buildGroups('engineering', engineeringLoaders)
} as const
