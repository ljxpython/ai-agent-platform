...
const workspaceBindingRows = computed(() => {
  ...
  return [
    ...
    {
      label: '治理链路',
      value: 'platform-web-vue → platform-api-v2 → LightRAG',
...
<template>
  <section class="pw-page-shell">
    ...
    >
      <template #actions>
        <BaseButton variant="secondary" :disabled="refreshing || !canAdmin" @click="refreshSpace">
          {{ refreshing ? '刷新中…' : '刷新状态' }}
        </BaseButton>
      </template>
    </PageHeader>
    ...
    <div class="mt-4 grid gap-4 xl:grid-cols-2">
      <SurfaceCard>
        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-gray-400 dark:text-dark-500">默认知识空间</div>
        <template v-if="space">
          <dl class="mt-4 space-y-3 text-sm text-gray-700 dark:text-dark-200">
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">显示名称</dt>
              <dd class="font-medium text-gray-900 dark:text-white">{{ space.display_name }}</dd>
            </div>
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">Provider</dt>
              <dd>{{ space.provider }}</dd>
            </div>
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">workspace_key</dt>
              <dd class="font-mono text-xs">{{ space.workspace_key }}</dd>
            </div>
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">状态</dt>
              <dd>{{ space.status }}</dd>
            </div>
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">服务地址</dt>
              <dd class="text-right">{{ space.service_base_url }}</dd>
            </div>
            <div class="flex items-start justify-between gap-4">
              <dt class="text-gray-400 dark:text-dark-500">更新时间</dt>
      ...
      <SurfaceCard>
        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-gray-400 dark:text-dark-500">workspace 映射摘要</div>
        <template v-if="space">
          <dl class="mt-4 space-y-3 text-sm text-gray-700 dark:text-dark-200">
            <div
            ...
            >
              <dt class="text-gray-400 dark:text-dark-500">{{ row.label }}</dt>
              <dd :class="row.valueClass">{{ row.value }}</dd>
            </div>
          </dl>
      ...
      <SurfaceCard>
        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-gray-400 dark:text-dark-500">服务健康</div>
        <template v-if="space?.health">
          <pre class="mt-4 overflow-auto rounded-2xl bg-gray-950/90 p-4 text-xs leading-6 text-white dark:bg-dark-950">{{ JSON.stringify(space.health, null, 2) }}</pre>
        </template>
        <p v-else class="mt-4 text-sm text-gray-500 dark:text-dark-300">
      ...
      <SurfaceCard>
        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-gray-400 dark:text-dark-500">运行说明 / 风险说明</div>
        <div class="mt-4 space-y-4 text-sm leading-6 text-gray-600 dark:text-dark-300">
          <div>
            <div class="font-medium text-gray-900 dark:text-white">运行说明</div>
            <ul class="mt-2 list-disc space-y-2 pl-5">
              ...
              <li>当前 human-facing 路径固定为 <code>platform-web-vue → platform-api-v2 → LightRAG</code>。</li>
            </ul>
          </div>
          <div>
            <div class="font-medium text-gray-900 dark:text-white">风险说明</div>
            <ul class="mt-2 list-disc space-y-2 pl-5">
