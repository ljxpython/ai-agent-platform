...
<template>
  <aside class="flex h-full min-h-0 flex-col border-l border-gray-200 bg-gray-50/90 dark:border-dark-800 dark:bg-dark-950/70">
    <div class="border-b border-gray-200 px-5 py-4 dark:border-dark-800">
    ...
    <div class="grid min-h-0 flex-1 lg:grid-cols-[180px_minmax(0,1fr)]">
      <div class="min-h-0 overflow-y-auto border-b border-gray-200 px-3 py-3 dark:border-dark-800 lg:border-b-0 lg:border-r">
        <div class="space-y-2">
          <button
            v-for="(entry, index) in entries"
            :key="entryId(entry, index)"
            type="button"
            class="block w-full rounded-xl border px-3 py-3 text-left transition-colors duration-150"
            :class="
              entryId(entry, index) === selectedEntryId
                ? 'border-primary-200 bg-white text-gray-900 dark:border-primary-900/40 dark:bg-dark-900 dark:text-white'
                : 'border-transparent text-gray-500 hover:border-gray-200 hover:bg-white hover:text-gray-900 dark:text-dark-300 dark:hover:border-dark-700 dark:hover:bg-dark-900 dark:hover:text-white'
            "
