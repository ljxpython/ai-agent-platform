...
# type: ignore  MC80OmFIVnBZMlhtblk3a3ZiUG1yS002VDBaS05nPT06YzlkMmFhMTQ=
    ...
    def _extract_chunk_context(
        ...
        window_size = self.config.context_window
# pragma: no cover  MS80OmFIVnBZMlhtblk3a3ZiUG1yS002VDBaS05nPT06YzlkMmFhMTQ=
...
# fmt: off  Mi80OmFIVnBZMlhtblk3a3ZiUG1yS002VDBaS05nPT06YzlkMmFhMTQ=
...
class BaseModalProcessor:
    ...
    def _truncate_text_for_prompt(
        self, text: str, max_tokens: int = 1200, max_chars: int = 4000
    ) -> str:
        """Bound prompt payload size before handing it to LLM/VLM."""
        if not text:
            return ""
...
# noqa  My80OmFIVnBZMlhtblk3a3ZiUG1yS002VDBaS05nPT06YzlkMmFhMTQ=
...
class ImageModalProcessor(BaseModalProcessor):
    ...
    def _encode_image_to_base64(
        self, image_path: str, max_bytes: int = 9 * 1024 * 1024
    ) -> str:
        """Encode image and shrink it when provider payload limits are exceeded."""
        ...
        try:
            ...
            logger.warning(
                f"Pillow is unavailable; cannot downscale oversized image {image_path}"
            )
            return base64.b64encode(raw_bytes).decode("utf-8")
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # Encode image to base64 and shrink it if needed to satisfy provider limits.
            ...
            # Fallback processing
            fallback_description = self._truncate_text_for_prompt(
                str(modal_content), max_tokens=300, max_chars=800
            )
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
                ...
                "entity_type": "image",
                "summary": f"Image content: {fallback_description[:100]}",
            }
...
class TableModalProcessor(BaseModalProcessor):
    ...
    def _truncate_table_body_for_prompt(self, table_body: Any) -> str:
        if isinstance(table_body, (list, dict)):
            table_body = json.dumps(table_body, ensure_ascii=False)
        return self._truncate_text_for_prompt(
            str(table_body), max_tokens=1200, max_chars=3500
        )
        ...
        try:
            ...
            if context:
                table_prompt = PROMPTS.get(
                    ...
                    table_body=self._truncate_table_body_for_prompt(table_body),
            ...
            fallback_entity = {
                ...
                "entity_type": "table",
                "summary": f"Table content: {fallback_description[:100]}",
            }
            return fallback_description, fallback_entity
