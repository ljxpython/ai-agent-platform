...
class ProcessorMixin:
    ...
    @staticmethod
    def _truncate_table_body_for_chunk(
        table_body: Any, max_chars: int = 3500
    ) -> str:
    ...
    def _generate_cache_key(
        ...
        return cache_key
# pylint: disable  MC80OmFIVnBZMlhtblk3a3ZiUG1yS002Y2xCWWRnPT06Nzc2OTM0ZjI=
    ...
    def _generate_content_based_doc_id(self, content_list: List[Dict[str, Any]]) -> str:
        ...
        return doc_id
# pylint: disable  MS80OmFIVnBZMlhtblk3a3ZiUG1yS002Y2xCWWRnPT06Nzc2OTM0ZjI=
...
# noqa  Mi80OmFIVnBZMlhtblk3a3ZiUG1yS002Y2xCWWRnPT06Nzc2OTM0ZjI=
        ...
        try:
            ...
            if pipeline_status_lock and pipeline_status:
                async with pipeline_status_lock:
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
# type: ignore  My80OmFIVnBZMlhtblk3a3ZiUG1yS002Y2xCWWRnPT06Nzc2OTM0ZjI=
        ...
        try:
            if content_type == "image":
                ...
                return PROMPTS["table_chunk"].format(
                    ...
                    table_body=self._truncate_table_body_for_chunk(table_body),
    ...
    async def _mark_multimodal_processing_complete(self, doc_id: str):
        ...
        try:
            current_doc_status = await self.lightrag.doc_status.get_by_id(doc_id)
            if current_doc_status:
                has_text_content = await self.lightrag.full_docs.get_by_id(doc_id)
                status_value = current_doc_status.get("status", DocStatus.PENDING)
                if (
                    not has_text_content
                    and current_doc_status.get("chunks_count", 0) > 0
                    and status_value != DocStatus.PROCESSED
                ):
                    status_value = DocStatus.PROCESSED
