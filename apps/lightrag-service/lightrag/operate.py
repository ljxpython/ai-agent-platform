...
def _split_candidate_file_paths(file_path: Any) -> list[str]:
    if not isinstance(file_path, str):
        return []
    return [part.strip() for part in file_path.split(GRAPH_FIELD_SEP) if part.strip()]


def _metadata_filters_match(
    metadata: dict[str, Any], metadata_filters: dict[str, Any] | None
) -> bool:
    if not metadata_filters:
        return True

    tags = metadata.get("tags", [])
    normalized_tags = set(_normalize_metadata_value(tags))

    tags_any = {
        str(tag).strip()
        for tag in metadata_filters.get("tags_any", []) or []
        if str(tag).strip()
    }
    ...
    tags_all = {
        str(tag).strip()
        for tag in metadata_filters.get("tags_all", []) or []
        if str(tag).strip()
    }
    ...
    expected_attributes = metadata_filters.get("attributes", {}) or {}
    if isinstance(expected_attributes, dict):
        for key, expected in expected_attributes.items():
            actual_values = set(_normalize_metadata_value(attributes.get(key)))
            expected_values = set(_normalize_metadata_value(expected))
            if expected_values and not actual_values.intersection(expected_values):
                return False

    return True


async def _load_doc_metadata_by_file_paths(
    file_paths: set[str],
    doc_status_storage: DocStatusStorage | None,
) -> dict[str, dict[str, Any]]:
    if not doc_status_storage or not file_paths:
        return {}

    metadata_by_path: dict[str, dict[str, Any]] = {}
    for file_path in file_paths:
        try:
            doc = await doc_status_storage.get_doc_by_file_path(file_path)
        except Exception as exc:
            logger.debug(
                "Failed to load doc metadata for file_path=%s during metadata filter: %s",
                file_path,
                exc,
            )
            continue
        ...
        if isinstance(metadata, dict):
            metadata_by_path[file_path] = metadata

    return metadata_by_path


async def _load_allowed_chunk_ids_by_file_paths(
    file_paths: set[str],
    doc_status_storage: DocStatusStorage | None,
) -> set[str]:
    if not doc_status_storage or not file_paths:
    ...
    for file_path in file_paths:
        try:
            ...
            logger.debug(
                "Failed to load chunk scope for file_path=%s during metadata filter: %s",
                file_path,
                exc,
            )
            continue
        ...
        chunk_values = doc.get("chunks_list", [])
        if isinstance(chunk_values, list):
            allowed_chunk_ids.update(
                str(chunk_id).strip()
                for chunk_id in chunk_values
...
def _item_matches_loaded_metadata_filters(
    item: dict[str, Any],
    ...
    file_paths = _split_candidate_file_paths(item.get("file_path"))
    if not file_paths:
        return False

    for file_path in file_paths:
        metadata = metadata_by_path.get(file_path)
        if metadata and _metadata_filters_match(metadata, metadata_filters):
            return True

    return False


def _filter_chunks_by_allowed_ids(
    ...
    for chunk in chunks:
        ...
        if chunk_id and str(chunk_id) in allowed_chunk_ids:
            filtered.append(chunk)
    return filtered


def _should_apply_hard_metadata_scope(
    metadata_filters: dict[str, Any] | None,
    strict_scope: bool,
) -> bool:
    return bool(metadata_filters) and bool(strict_scope)
...
async def kg_query(
    ...
    args_hash = compute_args_hash(
        ...
        json.dumps(query_param.metadata_filters or {}, sort_keys=True, ensure_ascii=False),
        json.dumps(query_param.metadata_boost or {}, sort_keys=True, ensure_ascii=False),
        bool(query_param.strict_scope),
    )
    ...
    if cached_result is not None:
        ...
        if hashing_kv and hashing_kv.global_config.get("enable_llm_cache"):
            queryparam_dict = {
                ...
                "metadata_boost": query_param.metadata_boost or {},
                "strict_scope": bool(query_param.strict_scope),
            }
            await save_to_cache(
                hashing_kv,
                CacheData(
...
async def _merge_all_chunks(
    ...
    if filtered_entities and text_chunks_db:
        ...
        entity_chunks = _filter_chunks_by_allowed_ids(entity_chunks, allowed_chunk_ids)
    ...
    if filtered_relations and text_chunks_db:
        ...
        relation_chunks = _filter_chunks_by_allowed_ids(
            relation_chunks, allowed_chunk_ids
        )

    vector_chunks = _filter_chunks_by_allowed_ids(vector_chunks, allowed_chunk_ids)
...
async def _build_context_str(
    ...
    final_data["metadata"]["retrieval_scope"] = {
        "metadata_filters": query_param.metadata_filters or {},
        "metadata_boost": query_param.metadata_boost or {},
        "strict_scope": bool(query_param.strict_scope),
    }
...
async def _build_query_context(
    ...
    metadata_filters = query_param.metadata_filters or {}
    allowed_chunk_ids: set[str] | None = None
    if _should_apply_hard_metadata_scope(
        metadata_filters, bool(query_param.strict_scope)
    ):
        candidate_paths: set[str] = set()
        for group_name in ("final_entities", "final_relations", "vector_chunks"):
            for item in search_result.get(group_name, []) or []:
                candidate_paths.update(_split_candidate_file_paths(item.get("file_path")))

        metadata_by_path = await _load_doc_metadata_by_file_paths(
            candidate_paths,
            doc_status_storage,
        )
        allowed_chunk_ids = await _load_allowed_chunk_ids_by_file_paths(
            {
                file_path
                for file_path, metadata in metadata_by_path.items()
                if _metadata_filters_match(metadata, metadata_filters)
            },
            doc_status_storage,
        )

        for group_name in ("final_entities", "final_relations", "vector_chunks"):
            search_result[group_name] = [
                item
                for item in search_result.get(group_name, []) or []
                if _item_matches_loaded_metadata_filters(
                    item,
                    metadata_filters,
                    metadata_by_path,
                )
        ...
        if search_result.get("chunk_tracking"):
            search_result["chunk_tracking"] = {
                chunk_id: tracking
                for chunk_id, tracking in search_result["chunk_tracking"].items()
                if chunk_id in (allowed_chunk_ids or set()) or tracking.get("source") != "C"
            }
...
async def naive_query(
    ...
    if _should_apply_hard_metadata_scope(
        ...
        candidate_paths = {
            path
            for chunk in chunks
            for path in _split_candidate_file_paths(chunk.get("file_path"))
        }
        metadata_by_path = await _load_doc_metadata_by_file_paths(
            candidate_paths,
            doc_status_storage,
    ...
    raw_data["metadata"]["retrieval_scope"] = {
        "metadata_filters": query_param.metadata_filters or {},
        "metadata_boost": query_param.metadata_boost or {},
        "strict_scope": bool(query_param.strict_scope),
    }
