...
class QueryParam:
    ...
    Intended to support generic metadata-aware retrieval such as:
    - tags_any / tags_all
    - attributes exact-match filters
    """

    metadata_boost: dict[str, Any] | None = None
    """Optional metadata-based ranking hints applied during retrieval.

    This is reserved for soft-preference / boost semantics and may be ignored by
    storage/query implementations that do not support ranking-time metadata bias.
    """

    strict_scope: bool = False
    """When True, retrieval should only use results that satisfy metadata_filters.

    When False, metadata filters may be treated as soft hints by higher layers.
    """
