...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=get_env_value("ENABLE_MULTIMODAL_PROCESSING", False, bool),
        help="Enable multimodal document processing with RAG-Anything",
    )
    parser.add_argument(
        "--multimodal-parser",
        type=str,
        default=get_env_value("MULTIMODAL_PARSER", "docling"),
        choices=["mineru", "docling"],
        help="Parser for multimodal documents (default: docling)",
    )
    parser.add_argument(
        "--multimodal-parse-method",
        type=str,
        default=get_env_value("MULTIMODAL_PARSE_METHOD", "auto"),
        choices=["auto", "ocr", "txt"],
        help="Parse method for multimodal documents",
    )
    parser.add_argument(
        "--multimodal-parser-output-dir",
        type=str,
        default=get_env_value("MULTIMODAL_PARSER_OUTPUT_DIR", "./parser_output"),
        help="Output directory for multimodal parser results",
    )
    parser.add_argument(
        "--vl-binding",
        type=str,
        default=get_env_value("VL_BINDING", None),
        choices=[None, "openai", "azure_openai", "gemini", "ollama"],
        help="Vision model binding type",
    )
    parser.add_argument(
        "--vl-model",
        type=str,
        default=get_env_value("VL_MODEL", None),
        help="Vision model name",
    )
    parser.add_argument(
        "--vl-binding-host",
        type=str,
        default=get_env_value("VL_BINDING_HOST", None),
        help="Vision model endpoint",
    )
    parser.add_argument(
        "--vl-binding-api-key",
        type=str,
        default=get_env_value("VL_BINDING_API_KEY", None),
        help="Vision model API key",
    )
    parser.add_argument(
        "--vl-follow-llm",
        action="store_true",
        default=get_env_value("VL_FOLLOW_LLM", True, bool),
        help="Allow VL_* settings to fall back to the main LLM_* settings",
    )
    ...
    args.enable_multimodal_processing = get_env_value(
        "ENABLE_MULTIMODAL_PROCESSING", False, bool
    )
    args.multimodal_parser = get_env_value("MULTIMODAL_PARSER", "docling")
    args.multimodal_parse_method = get_env_value("MULTIMODAL_PARSE_METHOD", "auto")
    args.multimodal_parser_output_dir = get_env_value(
        "MULTIMODAL_PARSER_OUTPUT_DIR", "./parser_output"
    )
    args.vl_follow_llm = get_env_value("VL_FOLLOW_LLM", True, bool)
    args.vl_binding = get_env_value("VL_BINDING", None)
    args.vl_model = get_env_value("VL_MODEL", None)
    args.vl_binding_host = get_env_value("VL_BINDING_HOST", None)
    args.vl_binding_api_key = get_env_value("VL_BINDING_API_KEY", None)

    if args.vl_follow_llm:
        args.vl_binding = args.vl_binding or args.llm_binding
        args.vl_model = args.vl_model or args.llm_model
        args.vl_binding_host = args.vl_binding_host or args.llm_binding_host
        args.vl_binding_api_key = (
            args.vl_binding_api_key or args.llm_binding_api_key
        )
