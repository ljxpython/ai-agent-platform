...
class QueryRequest(BaseModel):
    ...
    metadata_filters: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional generic metadata filters used to narrow retrieval scope.",
    )

    metadata_boost: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional generic metadata-based ranking hints used to prefer certain results.",
    )

    strict_scope: Optional[bool] = Field(
        default=None,
        description="If True, only results matching metadata_filters may be used during retrieval.",
    )
...
def create_query_routes(rag_resolver, api_key: Optional[str] = None, top_k: int = 60):
