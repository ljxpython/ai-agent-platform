...
from lightrag.api.raganything_integration import create_raganything_processor
from lightrag.api.routers.query_routes import create_query_routes
from lightrag.api.routers.graph_routes import create_graph_routes
from lightrag.api.routers.ollama_api import OllamaAPI
from lightrag.api.workspace_manager import (
    LightRAGWorkspaceManager,
    WorkspaceResources,
)
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            await workspace_manager.get_resources(None)
            ...
            await workspace_manager.finalize_all()
    ...
    if args.enable_multimodal_processing:

        def create_vision_model_func(vl_binding: str):
            try:
                if vl_binding == "openai":
                    from lightrag.llm.openai import openai_complete_if_cache

                    async def vision_openai_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                        ...
                        **kwargs,
                        ...
                        if messages:
                            return await openai_complete_if_cache(
                                ...
                                history_messages=[],
                                messages=messages,
                                api_key=args.vl_binding_api_key,
                                base_url=args.vl_binding_host,
                                timeout=llm_timeout,
                                **kwargs,
                            )
                        if image_data:
                        ...
                        return await create_llm_model_func(args.llm_binding)(
                            prompt, system_prompt, history_messages, **kwargs
                        )

                    return vision_openai_complete

                if vl_binding == "azure_openai":
                    from lightrag.llm.azure_openai import azure_openai_complete_if_cache

                    async def vision_azure_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                    ...
                    return vision_azure_complete

                if vl_binding == "gemini":
                    from lightrag.llm.gemini import gemini_complete_if_cache

                    async def vision_gemini_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                    ...
                    return vision_gemini_complete

                if vl_binding == "ollama":
                    from lightrag.llm.ollama import ollama_model_complete

                    async def vision_ollama_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                        ...
                        **kwargs,
                        ...
                        return await ollama_model_complete(
                            ...
                            host=args.vl_binding_host,
                            timeout=llm_timeout,
                            **kwargs,
                        )

                    return vision_ollama_complete
            except ImportError as exc:
                logger.error(f"Failed to import vision binding {vl_binding}: {exc}")
                return None

            logger.warning(f"Unsupported vision binding: {vl_binding}")
            return None

        if args.vl_binding and args.vl_model:
            vision_model_func = create_vision_model_func(args.vl_binding)
            if vision_model_func:
                logger.info(
                    f"Vision model initialized: {args.vl_model} using {args.vl_binding}"
                )
            else:
                logger.warning(
                    "Vision model initialization failed. Multimodal processing will use LLM fallback."
                )
        else:
            logger.warning(
                "Multimodal processing enabled without complete VL_* settings. Falling back to LLM where possible."
            )
    ...
    default_doc_manager = DocumentManager(args.input_dir, workspace=args.workspace)
    default_raganything_processor = None
    if global_args.enable_multimodal_processing:
        try:
            default_raganything_processor = create_raganything_processor(
                rag=rag,
                enable_multimodal=global_args.enable_multimodal_processing,
                parser=global_args.multimodal_parser,
                parse_method=global_args.multimodal_parse_method,
                parser_output_dir=global_args.multimodal_parser_output_dir,
                vision_model_func=vision_model_func,
            )
            if default_raganything_processor:
                logger.info(
                    f"RAG-Anything multimodal processing enabled with parser={global_args.multimodal_parser}"
                )
        except Exception as e:
            logger.warning(
                f"Failed to initialize RAG-Anything processor: {e}. Falling back to standard document processing."
            )
            default_raganything_processor = None

    def build_doc_manager(workspace: str) -> DocumentManager:
        return DocumentManager(args.input_dir, workspace=workspace)

    def build_raganything_processor(new_rag: LightRAG):
        if not global_args.enable_multimodal_processing:
            return None
        try:
            return create_raganything_processor(
                rag=new_rag,
                enable_multimodal=global_args.enable_multimodal_processing,
                parser=global_args.multimodal_parser,
                parse_method=global_args.multimodal_parse_method,
            ...
            logger.warning(
                f"Failed to initialize RAG-Anything processor for workspace '{new_rag.workspace}': {e}. Falling back to standard document processing."
            )
            return None

    workspace_manager = LightRAGWorkspaceManager(
        default_workspace=args.workspace,
        rag_factory=build_rag,
        doc_manager_factory=build_doc_manager,
        raganything_factory=build_raganything_processor,
    )

    async def register_default_resources() -> None:
        await workspace_manager.register_resources(
            workspace=args.workspace,
            resources=WorkspaceResources(
                workspace=args.workspace,
                rag=rag,
                doc_manager=default_doc_manager,
                raganything_processor=default_raganything_processor,
            ),
        )

    async def get_workspace_resources(request: Request) -> WorkspaceResources:
        workspace = get_workspace_from_request(request)
        return await workspace_manager.get_resources(workspace)

    async def get_request_scoped_rag(request: Request) -> LightRAG:
        return (await get_workspace_resources(request)).rag
    ...
    app.include_router(create_query_routes(get_request_scoped_rag, api_key, args.top_k))
    app.include_router(create_graph_routes(get_request_scoped_rag, api_key))
    ...
    async def get_status(request: Request):
        ...
        try:
            workspace = get_workspace_from_request(request)
            resources = await get_workspace_resources(request)
            default_workspace = get_default_workspace()
            if workspace is None:
                workspace = resources.workspace or default_workspace
