...
def _normalize_document_metadata(metadata: Any) -> dict[str, Any]:
...
class LightRAG:
    ...
    async def ainsert(
        ...
        Args:
            ...
            metadata_list: list of metadata dictionaries corresponding to each document
            track_id: tracking ID for monitoring processing status, if not provided, will be generated
    ...
    async def apipeline_enqueue_documents(
        ...
        if metadata_list is not None:
            if len(metadata_list) != len(input):
                raise ValueError(
                    "Number of metadata entries must match the number of documents"
                )
            normalized_metadata_list = [
                _normalize_document_metadata(metadata) for metadata in metadata_list
            ]
        else:
            normalized_metadata_list = [{} for _ in input]
        ...
        if ids is not None:
            ...
            unique_contents = {}
            for id_, doc, path, metadata in zip(
                ids, input, file_paths, normalized_metadata_list
            ):
            ...
            for doc, path, metadata in zip(input, file_paths, normalized_metadata_list):
                cleaned_content = sanitize_text_for_encoding(doc)
                if cleaned_content not in unique_content_with_paths:
                    unique_content_with_paths[cleaned_content] = (path, metadata)
        ...
        new_docs: dict[str, Any] = {
            id_: {
                ...
                "metadata": _normalize_document_metadata(content_data.get("metadata")),
            }
        ...
        full_docs_data = {
            doc_id: {
                ...
                "metadata": _normalize_document_metadata(contents[doc_id].get("metadata")),
            }
    ...
    async def _validate_and_fix_document_consistency(
        ...
        for doc_id, status_doc in to_process_docs.items():
            ...
            has_multimodal_chunks = bool(preserved_chunks_list) or preserved_chunks_count > 0
            metadata = getattr(status_doc, "metadata", {}) or {}
            multimodal_only = False
            if isinstance(metadata, dict):
                multimodal_only = bool(metadata.get("multimodal_only"))
            if not content_data:
                ...
                elif has_multimodal_chunks or multimodal_only:
                    # Pure multimodal documents may not have a full_docs row but are still valid
                    continue
                else:
            ...
            if content_data:  # Document passes consistency check
                ...
                if hasattr(status_doc, "status") and status_doc.status in [
                    ...
                    docs_to_reset[doc_id] = {
                        ...
                        # Clear error messages but preserve user metadata
                        "error_msg": "",
    ...
    async def aquery_data(
        ...
        data_param = QueryParam(
            ...
            metadata_boost=param.metadata_boost,
            strict_scope=param.strict_scope,
        )
        ...
        if query_result is None:
            ...
            final_data: dict[str, Any] = {
                ...
                "metadata": {
                    ...
                    "retrieval_scope": {
                        "metadata_filters": data_param.metadata_filters or {},
                        "metadata_boost": data_param.metadata_boost or {},
                        "strict_scope": bool(data_param.strict_scope),
                    },
                },
            }
            ...
            if query_result is None:
                return {
                    ...
                    "metadata": {
                        ...
                        "mode": param.mode,
                        "retrieval_scope": {
                            "metadata_filters": param.metadata_filters or {},
                            "metadata_boost": param.metadata_boost or {},
                            "strict_scope": bool(param.strict_scope),
                        },
                    },
                    "llm_response": {
