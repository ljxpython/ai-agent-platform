...
export async function listMembers(projectId: string, options?: { query?: string }): Promise<ManagementProjectMember[]> {
...
export async function upsertMember(payload: {
  ...
  return client.post<ManagementProjectMember>(`/_management/projects/${payload.projectId}/members`, {
...
export async function deleteMember(projectId: string, userId: string): Promise<{ ok: boolean }> {
  const client = createManagementApiClient();
  if (!client) {
    throw new Error("management_api_unavailable");
  }
  return client.del<{ ok: boolean }>(`/_management/projects/${projectId}/members/${userId}`);
}
