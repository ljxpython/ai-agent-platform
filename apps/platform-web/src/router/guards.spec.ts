...
describe("router guards", () => {
  ...
  it("does not reuse previous project permission when the target access fails", async () => {
    mocks.workspaceStore.contextLoaded = true;
    mocks.workspaceStore.currentProjectAccess = {
      project_id: "project-a",
      permissions: ["project.runtime.read"],
    };
    mocks.workspaceStore.setProjectId.mockRejectedValueOnce(
      new Error("forbidden"),
    );
    const fullPath = "/workspace/projects/project-b/chat/thread-b";
    const result = await captureGuard()({
      path: fullPath,
      fullPath,
      params: { projectId: "project-b" },
