...
const testcaseTarget = computed(() =>
  resolveChatTarget({
    targetType: 'graph',
    graphId: 'test_case_agent_v2',
    graphName: 'Test Case Agent V2',
...
async function loadOverview(projectId: string) {
  ...
  try {
    ...
    overview.value = null
    error.value = loadError instanceof Error ? loadError.message : 'Testcase Agent V2 概览加载失败'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="pw-page-shell pw-chat-page-shell">
    <TestcaseV2WorkspaceNav :compact="compactChrome" />

    <StateBanner
      v-if="error"
      title="Testcase Agent V2 概览加载失败"
      :description="error"
      variant="warning"
    />
    ...
    <EmptyState
      ...
      icon="project"
      title="请先选择项目"
      description="Testcase Agent V2 工作区也是项目级入口。没有项目上下文，文档解析、测试用例和生成会话都没法稳定落地。"
    />
    ...
    <BaseChatTemplate
      ...
      :display="{
        title: 'Testcase Agent V2 · AI 对话生成',
        description:
          '这里固定绑定 `test_case_agent_v2`，用于发起文档解析、提炼和测试用例生成对话。',
