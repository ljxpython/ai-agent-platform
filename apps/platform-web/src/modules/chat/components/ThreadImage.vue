...
<template>
  <div class="thread-image-container my-2 max-w-md rounded-lg border border-slate-200 bg-slate-50 p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
    ...
    >
      <BaseIcon name="refresh" class="h-6 w-6 animate-spin mr-2" />
    ...
    >
      <BaseIcon name="alert" class="h-6 w-6 text-red-500 mb-1" />
      <span class="text-xs font-medium text-red-600 dark:text-red-400">{{ error }}</span>
      <span class="text-[11px] text-slate-500 truncate max-w-xs mt-1" :title="imageRef.path">{{ imageRef.path }}</span>
      <button
        type="button"
    ...
    <div v-else-if="blobUrl" class="group relative overflow-hidden rounded-md">
      <img
        :src="blobUrl"
        :alt="alt || filename"
