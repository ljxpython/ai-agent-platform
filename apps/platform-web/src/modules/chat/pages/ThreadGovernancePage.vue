...
const state =
  shallowRef<
    Awaited<ReturnType<ReturnType<typeof createSessionService>["state"]>>
  >();
...
const canGovern = computed(
  () =>
    authorization.can("platform.super_admin.manage") ||
    (!!projectId.value &&
      authorization.can("project.runtime.write", projectId.value)),
);
...
const messages = computed(() => {
  ...
  return Array.isArray(values.messages)
    ? values.messages.map((value, index) => {
        const message = asObject(value);
        return {
          key: String(message.id || index),
          role: String(message.role || message.type || "消息"),
          blocks: contentItems(message.content, String(index)),
        };
      })
    : [];
});
...
async function refresh() {
  ...
  try {
    ...
    clearContent();
    error.value =
      cause instanceof Error ? cause.message : "读取失败，请先申请临时访问";
  } finally {
    if (current === epoch && !disposed) busy.value = false;
  }
}
function locate() {
...
async function approve(drafts: Record<string, ReviewDraft[]>) {
  if (
    ...
    busy.value ||
    !canGovern.value ||
    !hasThreadAction(thread.value, "approve")
  )
    return;
  const current = epoch;
  const id = target.value.threadId;
  ...
  try {
    ...
    if (current !== epoch || disposed) return;
    if (
      latest.length !== before.length ||
      before.some(
        (item) =>
          !latest.some(
            (value) =>
              value.id === item.id && value.fingerprint === item.fingerprint,
          ),
      )
    )
...
<template>
  <section class="pw-page-shell">
    ...
    />
    <SurfaceCard>
      <form class="space-y-4" @submit.prevent="locate">
        <label v-if="!routeProject" class="block text-sm"
          >项目<BaseSelect
        ...
        /></label>
        <BaseButton type="submit" :disabled="busy || !canGovern || !projectId">
          定位会话
        </BaseButton>
      </form>
    </SurfaceCard>
    ...
    <SurfaceCard v-if="target && canGovern">
      <div class="flex flex-wrap items-center gap-3">
        <ThreadAccessControl
          ...
          :show-takeover-button="true"
          @updated="accessUpdated"
        />
        <BaseButton variant="secondary" :disabled="busy" @click="refresh">
          重新读取
        </BaseButton>
        <BaseButton
          variant="secondary"
      ...
      <p v-if="!thread" class="mt-3 text-sm">
    ...
    <template v-if="thread && target && canGovern">
      ...
      <ApprovalPanel
        ...
        @submit="approve"
      />
      <SurfaceCard v-for="message in messages" :key="message.key">
    ...
    >
      ...
      <BaseButton :disabled="busy || !canGovern" @click="remove">
        确认删除
      </BaseButton>
    </BaseDialog>
  </section>
