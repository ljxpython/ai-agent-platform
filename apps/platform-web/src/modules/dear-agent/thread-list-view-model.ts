export type ChatThreadStatusFilter =
  | "all"
  | "idle"
  | "busy"
  | "interrupted"
...
export function getChatThreadGroupKey(
  item: Pick<ChatThreadSummaryItem, "status" | "updatedAt">,
  now = new Date(),
) {
...
export function buildChatThreadListView(options: {
  ...
  const filteredItems = options.items.filter((item) => {
    const matchesStatus =
      normalizedStatus === "all" ? true : item.status === normalizedStatus;
  ...
  for (const key of order) {
    const items = filteredItems.filter(
      (item) => getChatThreadGroupKey(item, now) === key,
    );
    if (items.length > 0) {
      groups.push({
        key,
