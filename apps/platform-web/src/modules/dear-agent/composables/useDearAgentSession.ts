...
export function useDearAgentSession(options: {
  projectId: string;
  userId?: string;
  graphId: string;
  agentId?: string;
  ...
  watch(
    ...
    (len, prevLen) => {
      if (len > (prevLen ?? 0) && !disposed && run.value?.status !== "interrupted") {
        void verify(false);
      }
    },
  );
  const reviews = computed(() => {
    ...
    return parseReviews(stream.interrupts.value);
  });
  const rawClarifications = computed(() =>
    parseClarifications(stream.interrupts.value),
  );
  ...
  async function queueMessage(
    ...
    // 2. 如果本地缓存未命中，向服务端获取处于 running/pending 态的运行
    if (!targetRunId && (busy.value || queueOptions?.allowFallbackSend === false)) {
      try {
        const list = await service.client?.runs?.list?.(threadId.value, { limit: 5 }) ?? await service.runs(threadId.value);
    ...
    //    但若当前是从 send() 的 409 冲突分支降级进来（allowFallbackSend === false），绝不反向递归调用 send()！
    if (!targetRunId) {
      if (queueOptions?.allowFallbackSend === false) {
        return false;
      }
  ...
  async function send(
    ...
    try {
      ...
      let attempts = 0;
      const maxAttempts = sendOptions?.fromQueue ? 1 : 2;
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          if (is409) {
            ...
            console.warn("[session] 发送消息遇到服务端运行中冲突(409)，同步后台运行状态并交由前端队列排队等待");
            error.value = "";
            try {
              (stream.error as unknown as { value: unknown }).value = null;
            } catch {
