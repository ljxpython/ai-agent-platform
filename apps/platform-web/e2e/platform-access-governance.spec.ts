...
test.skip(
  ...
  "Requires the isolated governance_server.py fixture",
);
...
async function login(page: Page, name: string) {
  ...
  await page
    ...
    .fill(`governance-${name}`);
  await page.locator('input[type="password"]').fill(password);
  await page.locator('button[type="submit"]').click();
  await expect(page).toHaveURL(/\/workspace\//, { timeout: 30_000 });
}

test("private sharing and no-project administrator takeover/delete cross the real platform gateway", async ({
  page,
  request,
}) => {
  const fixture = JSON.parse(
    readFileSync("/tmp/platform-governance-e2e.json", "utf8"),
  ) as { project_id: string; users: Record<string, string> };
  ...
  const created = await request.post(`${apiUrl}/api/langgraph/threads`, {
    ...
    data: {
      metadata: {
        title: "Governance private fixture",
        harness: "platform-governance-browser",
      },
    },
  });
  ...
  try {
    ...
    expect((await request.get(path, { headers: headers(peer) })).status()).toBe(
      403,
    );
    expect(
      (await request.get(path, { headers: headers(admin) })).status(),
    ).toBe(403);
    const share = await request.put(`${path}/shares`, {
      headers: headers(owner),
      data: { user_id: fixture.users.peer, actions: ["read"] },
    });
    expect(share.ok()).toBeTruthy();
    expect(
      (await request.get(path, { headers: headers(peer) })).ok(),
    ).toBeTruthy();
    expect(
      (
    ...
    expect(
      (
        await request.put(`${path}/shares`, {
          headers: headers(owner),
          data: { user_id: fixture.users.peer, actions: [] },
        })
      ).ok(),
    ).toBeTruthy();
    expect((await request.get(path, { headers: headers(peer) })).status()).toBe(
    ...
    await page
      .getByRole("button", { name: "Governance E2E", exact: true })
      .click();
    ...
    await expect(
      page.getByRole("heading", { name: "Governance private fixture" }),
    ).toBeHidden();
    await page.getByRole("button", { name: "临时访问", exact: true }).click();
    await page
      .getByPlaceholder("例如：SEC-2026-0922 或 TICKET-8891")
      .fill("E2E-PRIVATE-001");
    await page
      .getByPlaceholder(
        "请详细说明接管此会话的具体原因与必要性（至少10个字符）…",
      )
      .fill("用户明确请求排查本次专用测试会话");
    await page
      .getByRole("button", { name: "申请临时读取", exact: true })
      .click();
    ...
    expect(
      (await request.get(path, { headers: headers(admin) })).ok(),
    ).toBeTruthy();
    await page.screenshot({
      path: "/tmp/governance-admin-desktop.png",
      fullPage: true,
    });
...
test("mobile operator lands on governance, synchronizes catalog, and cannot enter private governance or execute", async ({
  page,
  request,
}) => {
  ...
  await page
    ...
    .click();
  await page
    .getByRole("button", { name: "同步 Graph 目录", exact: true })
    .click();
  await expect(page.getByText(/Graph全局目录已刷新/)).toBeVisible({
    timeout: 30_000,
  });
  await page.screenshot({
    path: "/tmp/governance-operator-mobile.png",
    fullPage: true,
  });
  await page.goto(`${webUrl}/workspace/thread-governance`);
...
test("user creation retains the account when project binding fails and permits later binding", async ({
  page,
  request,
}) => {
  const fixture = JSON.parse(
  ...
  await page.getByRole("button", { name: "请选择项目", exact: true }).click();
  await page
    .getByRole("listbox")
    .getByRole("button", { name: "Governance E2E" })
    .click();
  await page.route(
    `**/api/projects/${fixture.project_id}/members/*`,
    (route) =>
      route.fulfill({
        status: 503,
        ...
        body: JSON.stringify({
          error: {
            ...
            message: "Temporary membership outage",
          },
        }),
      }),
    { times: 1 },
  ...
  await page
    .getByRole("button", { name: "创建用户并加入 1 个项目", exact: true })
    .click();
  const created = await createdResponse;
  expect(created.ok()).toBeTruthy();
  const user = await created.json();
  await expect(
    page.getByText(/已创建成功，但有 1 个项目绑定失败/),
  ).toBeVisible();
  await expect(page.getByPlaceholder("请输入至少 8 位密码")).toHaveValue("");
  await expect(page.getByPlaceholder("请再次输入密码")).toHaveValue("");
  const token = await identity(request, name);
  expect(
    (
      await request.post(`${apiUrl}/api/langgraph/threads`, {
        headers: {
          authorization: `Bearer ${token}`,
          "x-project-id": fixture.project_id,
        },
  ...
  expect(
    (
      await request.put(
        `${apiUrl}/api/projects/${fixture.project_id}/members/${user.id}`,
        {
          headers: { authorization: `Bearer ${provisioner}` },
  ...
  expect(
    (
      await request.delete(`${apiUrl}/api/langgraph/threads/${threadId}`, {
        headers,
      })
    ).ok(),
  ).toBeTruthy();
...
test("announcement editing keeps its scope and global governance excludes project notices", async ({
  page,
  request,
}) => {
  const fixture = JSON.parse(
  ...
  const global = await request.post(`${apiUrl}/api/announcements`, {
    headers,
    data: { title, body: "Governance browser fixture", scope_type: "global" },
  });
  const project = await request.post(`${apiUrl}/api/announcements`, {
    headers,
    data: {
      title: projectTitle,
      scope_type: "project",
      scope_project_id: fixture.project_id,
    },
  });
  ...
  try {
    ...
    await expect(
      page.getByRole("button", { name: "范围", exact: true }),
    ).toBeDisabled();
    await expect(
    ...
    await expect(
      page.getByRole("row").filter({ hasText: `${title} edited` }),
    ).toBeVisible();
    const operator = await identity(request, "operator");
    expect(
      (
        await request.patch(`${apiUrl}/api/announcements/${projectId}`, {
          headers: {
            authorization: `Bearer ${operator}`,
            "x-project-id": fixture.project_id,
          },
    ...
    expect(
      (
        await request.patch(`${apiUrl}/api/announcements/${globalId}`, {
          headers,
          data: { scope_type: "project", scope_project_id: fixture.project_id },
        })
      ).status(),
    ).toBe(400);
  } finally {
    for (const id of [globalId, projectId]) {
      expect(
        (
          await fetch(`${apiUrl}/api/announcements/${id}`, {
            method: "DELETE",
            headers,
          })
        ).ok,
...
test("operator cannot mutate a high-privilege service account through UI or API", async ({
  page,
  request,
}) => {
  const admin = await identity(request, "superadmin");
  ...
  const response = await request.post(`${apiUrl}/api/service-accounts`, {
    ...
    data: { name, platform_roles: ["platform_super_admin"] },
  });
  expect(response.ok()).toBeTruthy();
  ...
  await expect(
    card.getByText("高权限账号：仅平台管理员可编辑、停用或管理 Token"),
  ).toBeVisible();
  ...
  expect(
    (
      await request.patch(`${apiUrl}/api/service-accounts/${account.id}`, {
        headers,
  ...
  expect(
    (
      await request.post(
        `${apiUrl}/api/service-accounts/${account.id}/tokens`,
        {
          headers,
          data: { name: "denied-token" },
        },
      )
    ).status(),
  ).toBe(403);
...
test("private child resources deny peers and sharing never grants terminal or full access", async ({
  request,
}) => {
  const fixture = JSON.parse(
    readFileSync("/tmp/platform-governance-e2e.json", "utf8"),
  ...
  try {
    for (const child of [
      ...
      "/capabilities",
      "/workspace/tree",
      "/workspace/content?path=/workspace/private.txt",
      "/workspace/preview?path=/workspace/private.txt",
      "/workspace/zip",
      "/artifacts",
      "/images/content?path=/workspace/uploads/private.png",
      "/files/content?path=/workspace/uploads/private.pdf",
      "/terminals",
      "/dear/memory",
    ]) {
      expect(
        (
          await request.get(`${path}${child}`, { headers: headers(peer) })
        ).status(),
        child,
      ).toBe(403);
    }
    expect(
      (
        await request.post(`${path}/runs/stream`, {
          headers: headers(peer),
          data: {},
        })
      ).status(),
    ...
    expect((await shared.json()).metadata.allowed_actions.sort()).toEqual([
      "comment",
      "edit",
      "read",
    ]);
    expect(
      (
        await request.patch(`${path}/access-policy`, {
          headers: headers(peer),
          data: { access_policy: "full_access" },
        })
      ).status(),
    ).toBe(403);
    expect(
      (
        await request.post(`${path}/terminals`, {
          headers: headers(peer),
          data: {
    ...
    expect(
      (await request.delete(path, { headers: headers(peer) })).status(),
    ).toBe(403);
  } finally {
    expect(
      (await fetch(path, { method: "DELETE", headers: headers(owner) })).ok,
    ).toBeTruthy();
  }
});

test("service account needs an explicit project grant and project sharing, then revocation is immediate", async ({
  request,
}) => {
  const fixture = JSON.parse(
    readFileSync("/tmp/platform-governance-e2e.json", "utf8"),
  ...
  const account = await request.post(`${apiUrl}/api/service-accounts`, {
    headers: adminHeaders,
    data: {
      name: `Governance grant ${Date.now()}`,
      platform_roles: ["platform_viewer"],
    },
  });
  expect(account.ok()).toBeTruthy();
  const accountPath = `${apiUrl}/api/service-accounts/${(await account.json()).id}`;
  const issued = await request.post(`${accountPath}/tokens`, {
    headers: adminHeaders,
    data: { name: "fixture" },
  });
  ...
  const serviceHeaders = {
    "x-platform-api-key": credentials.plain_text_token,
    "x-project-id": fixture.project_id,
  };
  const created = await request.post(`${apiUrl}/api/langgraph/threads`, {
  ...
  try {
    expect(
      (await request.get(path, { headers: serviceHeaders })).status(),
    ).toBe(403);
    expect(
      (
        await request.put(
          `${accountPath}/project-grants/${fixture.project_id}`,
          {
            headers: adminHeaders,
            data: { role: "project_executor" },
          },
    ...
    expect(
      shared.ok(),
      `${shared.status()} ${await shared.text()}`,
    ).toBeTruthy();
    expect((await shared.json()).metadata.allowed_actions.sort()).toEqual([
      "comment",
      "read",
    ...
    expect(
      (
        await request.delete(`${accountPath}/tokens/${credentials.token.id}`, {
          headers: adminHeaders,
        })
      ).ok(),
    ).toBeTruthy();
    ...
    expect(
      (await fetch(path, { method: "DELETE", headers: ownerHeaders })).ok,
    ).toBeTruthy();
  }
});

test("platform viewer can read global model inventory but cannot create users or enter project governance", async ({
  page,
  request,
}) => {
  const fixture = JSON.parse(
  ...
  await expect(
    page.getByRole("heading", { name: "全局模型连接", exact: true }),
  ).toBeVisible();
  const viewer = await identity(request, "viewer");
  const headers = { authorization: `Bearer ${viewer}` };
  expect(
    (
      await request.get(`${apiUrl}/api/runtime/platform-models`, { headers })
    ).ok(),
  ).toBeTruthy();
  expect(
    (
  ...
  await page.goto(
    `${webUrl}/workspace/projects/${fixture.project_id}/thread-governance`,
  );
  await expect(page).toHaveURL(/access-unavailable/, { timeout: 30_000 });
});

test("project editor can create private threads but cannot edit project security policy", async ({
  page,
  request,
}) => {
  const fixture = JSON.parse(
  ...
  const access = await request.get(
    `${apiUrl}/api/projects/${fixture.project_id}/access`,
    { headers },
  );
  ...
  expect(
    (
      await request.put(
        `${apiUrl}/api/projects/${fixture.project_id}/runtime-policies/graphs/00000000-0000-4000-8000-000000000001`,
        {
          headers,
...
test("removing membership rejects new requests and clears both active browser tabs on focus", async ({
  ...
  try {
    await page.goto(url);
    await expect(
      page.getByRole("heading", { name: "请选择一个对话智能体" }),
    ).toBeVisible({ timeout: 30_000 });
    await other.goto(url);
    await expect(
      other.getByRole("heading", { name: "请选择一个对话智能体" }),
    ).toBeVisible({ timeout: 30_000 });
    await expect(page).toHaveURL(url);
    await expect(other).toHaveURL(url);
    expect(
      (await request.delete(membership, { headers: adminHeaders })).ok(),
    ).toBeTruthy();
    expect(
      (
        await request.post(`${apiUrl}/api/langgraph/threads`, {
    ...
    for (const tab of [page, other]) {
      ...
      await expect(
        tab.getByText(/^(当前页面权限已失效|无法访问此页面)$/),
      ).toBeVisible({ timeout: 30_000 });
      await expect(tab.locator(".pw-chat-page-shell")).toHaveCount(0);
    }
  } finally {
    const restored = await fetch(membership, {
      method: "PUT",
