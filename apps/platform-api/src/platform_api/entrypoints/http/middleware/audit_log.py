...
from platform_api.modules.audit.http_resolution import AuditHttpRequest
from platform_api.modules.audit.http_writer import (
...
def _should_capture_response(response: Response) -> bool:
    content_type = clean_str(response.headers.get("content-type")) or clean_str(
        response.media_type
    )
...
def _write_audit_event(
    *,
    ...
    actor = (
        audit_actor
        if isinstance(audit_actor, ActorContext)
        else (context.actor if context is not None else None)
    )
        ...
        command=WriteHttpAuditCommand(
            ...
            request=AuditHttpRequest(
                ...
                response_content_length=getattr(
                    request.state, "response_content_length", None
                ),
    ...
    async def audit_log_middleware(request: Request, call_next):
        ...
        async def audited_body_iterator() -> AsyncIterator[bytes]:
            ...
            try:
                ...
                if chunks:
                    request.state.audit_response_payload = _parse_json_bytes(
                        b"".join(chunks)
                    )
                duration_ms = max(
