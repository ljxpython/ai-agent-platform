...
class IdentityService:
    ...
    def _build_repository(self, session: Session) -> SqlAlchemyIdentityRepository:
    ...
    def _issue_session_tokens(
        ...
        *,
        ...
        repository.create_refresh_token(
            ...
            expires_at=_now()
            + timedelta(seconds=self._settings.jwt_refresh_ttl_seconds),
        )
    ...
    def login(self, command: LoginCommand) -> tuple[AuthenticatedSession, ActorContext]:
        session_factory = self._require_session_factory()
        authenticated: tuple[AuthenticatedSession, ActorContext] | None = None
        with session_scope(session_factory) as session:
            repository = self._build_repository(session)
            ...
            if not password_valid:
                ...
                session = AuthenticatedSession(
                    tokens=tokens, user=self._user_profile(user)
                )
                authenticated = (
                    session,
                    ActorContext(
    ...
    def refresh(self, command: RefreshSessionCommand) -> SessionTokens:
        session_factory = self._require_session_factory()
        try:
        ...
        if result is None:
            raise PlatformApiError(
                code=error_code, status_code=401, message=error_message
            )
        return result
    ...
    def get_current_user(self, actor: ActorContext) -> UserProfile:
        session_factory = self._require_session_factory()
        user_id = _parse_user_id(actor.user_id)
        with session_scope(session_factory) as session:
            repository = self._build_repository(session)
    ...
    def update_current_user(
        ...
        *,
        ...
        with session_scope(session_factory) as session:
            ...
            next_email = (
                next_email_raw.strip() if isinstance(next_email_raw, str) else None
            )
            if next_email == "":
                next_email = None
