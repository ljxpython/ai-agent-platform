...
from platform_api.modules.iam.domain import PlatformRole
...
def _resolve_platform_roles_for_update(
    *,
    ...
    roles = set(
        current_roles
        if command.platform_roles is None
        else _normalize_platform_roles(command.platform_roles)
    )
...
class UsersService:
    ...
    def list_users(
        ...
        *,
        ...
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_USER_READ
        )
        ...
        with session_scope(session_factory) as session:
            ...
            return UserPage(
                items=[self._user_item(item) for item in items], total=total
            )
    ...
    def create_user(
        ...
        *,
        ...
        session_factory = self._require_session_factory()
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_USER_CREATE
        )
        platform_roles = _resolve_platform_roles_for_create(command)
        if platform_roles:
            self._require_permission(
                actor=actor, permission=PermissionCode.PLATFORM_USER_ROLE_WRITE
            )
        with session_scope(session_factory) as session:
            repository = SqlAlchemyUsersRepository(session)
        ...
        *,
        ...
        with session_scope(session_factory) as session:
            ...
            if command.username is not None:
                self._require_permission(
                    actor=actor, permission=PermissionCode.PLATFORM_USER_PROFILE_WRITE
                )
            ...
            next_status = (
                command.status.value if command.status is not None else current.status
            )
            ...
            if (
                ...
                and (next_status != "active" or not next_is_super_admin)
            ...
            password_hash = (
                hash_password(command.password) if command.password else None
            )
    ...
    def reset_password(
        ...
        *,
        ...
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_USER_CREDENTIAL_RESET
        )
        user_uuid = parse_uuid(user_id, code="invalid_user_id")
