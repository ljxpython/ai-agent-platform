...
def _to_service_account(record: ServiceAccountRecord) -> StoredServiceAccount:
    return StoredServiceAccount(
        ...
        platform_roles=tuple(
            sorted(str(role) for role in (record.platform_roles_json or []))
        ),
...
class SqlAlchemyServiceAccountsRepository:
    ...
    def get_service_account_by_name(self, name: str) -> StoredServiceAccount | None:
        stmt = (
            select(ServiceAccountRecord)
            .where(ServiceAccountRecord.name == name)
            .limit(1)
        )
    ...
    def list_tokens(
        self, *, service_account_id: UUID
    ) -> list[StoredServiceAccountToken]:
        stmt = (
    ...
    def list_project_grants(
        self, *, service_account_id: UUID
    ) -> list[StoredServiceAccountProjectGrant]:
    ...
    def delete_project_grant(
        self, *, service_account_id: UUID, project_id: UUID
    ) -> bool:
        stmt = select(ServiceAccountProjectGrantRecord).where(
            ServiceAccountProjectGrantRecord.service_account_id == service_account_id,
            ServiceAccountProjectGrantRecord.project_id == project_id,
    ...
    def summarize(self) -> dict[str, int]:
        total_accounts = int(
            self.session.scalar(select(func.count()).select_from(ServiceAccountRecord))
            or 0
        )
        active_accounts = int(
            self.session.scalar(
