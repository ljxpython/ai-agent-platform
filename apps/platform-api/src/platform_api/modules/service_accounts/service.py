...
from platform_api.core.security import hash_password, verify_password
from platform_api.modules.iam.application import (
    AuthorizationRequest,
    IamPolicyEngine,
    PermissionCode,
)
from platform_api.modules.iam.domain import PlatformRole, ProjectRole
...
def _normalize_name(value: str) -> str:
    ...
    if not normalized:
        raise BadRequestError(
            code="invalid_service_account_name", message="Invalid service account name"
        )
    return normalized


def _normalize_roles(
    value: tuple[PlatformRole, ...] | tuple[str, ...],
) -> tuple[str, ...]:
    normalized = sorted(
        {
            str(role.value if isinstance(role, PlatformRole) else role).strip()
            for role in value
            if str(role).strip()
        }
    )
...
class ServiceAccountsService:
    ...
    def _require_permission(
        self, *, actor: ActorContext, permission: PermissionCode
    ) -> None:
    ...
    def _service_account_item(
        self, item, *, tokens: list[ServiceAccountTokenItem]
    ) -> ServiceAccountItem:
    ...
    def list_service_accounts(
        ...
        *,
        ...
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_SERVICE_ACCOUNT_READ
        )
        with session_scope(session_factory) as session:
            ...
            for item in items:
                tokens = [
                    self._token_item(token)
                    for token in repository.list_tokens(service_account_id=item.id)
                ]
    ...
    def create_service_account(
        ...
        *,
        ...
        session_factory = self._require_session_factory()
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_SERVICE_ACCOUNT_WRITE
        )
    ...
    def update_service_account(
        ...
        *,
        ...
        with session_scope(session_factory) as session:
            ...
            tokens = [
                self._token_item(token)
                for token in repository.list_tokens(service_account_id=updated.id)
            ]
    ...
    def list_project_grants(
        ...
        *,
        ...
        self._require_permission(
            actor=actor, permission=PermissionCode.PLATFORM_SERVICE_ACCOUNT_GRANT_WRITE
        )
        account_uuid = parse_uuid(service_account_id, code="invalid_service_account_id")
        with session_scope(self._require_session_factory()) as session:
            ...
            return [
                ...
                for item in repository.list_project_grants(
                    service_account_id=account_uuid
                )
            ]
    ...
    def upsert_project_grant(
        ...
        *,
        ...
        with session_scope(self._require_session_factory()) as session:
            repository = SqlAlchemyServiceAccountsRepository(session)
            account = repository.get_service_account_by_id(account_uuid)
            project = SqlAlchemyProjectsRepository(session).get_project_by_id(
                project_uuid
            )
    ...
    def delete_project_grant(
        ...
        *,
        ...
        project_uuid = parse_uuid(project_id, code="invalid_project_id")
        with session_scope(self._require_session_factory()) as session:
            deleted = SqlAlchemyServiceAccountsRepository(session).delete_project_grant(
                service_account_id=account_uuid,
                project_id=project_uuid,
            )
            if not deleted:
                raise NotFoundError(
                    code="service_account_project_grant_not_found",
                    message="Project grant not found",
                )

    def authenticate_api_key(
        self, api_key: str, *, project_id: str | None = None
    ) -> ActorContext | None:
