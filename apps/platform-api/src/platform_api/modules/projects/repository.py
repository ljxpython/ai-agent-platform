...
class SqlAlchemyProjectsRepository:
    ...
    def list_projects(
        ...
        *,
        ...
        stmt = (
            base_stmt.order_by(desc(ProjectRecord.created_at))
            .offset(offset)
            .limit(limit)
        )
    ...
    def list_projects_for_user(
        ...
        *,
        ...
        base_stmt = (
            ...
            .join(
                ProjectMemberRecord, ProjectMemberRecord.project_id == ProjectRecord.id
            )
            .where(
        ...
        *,
        ...
        rows = list(
            self.session.scalars(
                base_stmt.order_by(UserRecord.username.asc())
                .offset(offset)
                .limit(limit)
            ).all()
        )
        total = int(
            self.session.scalar(select(func.count()).select_from(base_stmt.subquery()))
            or 0
        )
        return [
            StoredProjectMemberCandidate(
                user_id=row.id, username=row.username, email=row.email
            )
            for row in rows
        ], total
