...
from platform_api.modules.iam.application.policies import PROJECT_PERMISSION_MAP
...
class ProjectsService:
    ...
    def create_project(
        ...
        *,
        ...
        self._policy_engine.require(
            actor=actor,
            authorization=AuthorizationRequest(
                permission=PermissionCode.PLATFORM_PROJECT_CREATE
            ),
        )
        with session_scope(session_factory) as session:
            repository = SqlAlchemyProjectsRepository(session)
    ...
    def delete_project(
        ...
        *,
        ...
        self._policy_engine.require(
            actor=actor,
            authorization=AuthorizationRequest(
                permission=PermissionCode.PLATFORM_PROJECT_WRITE
            ),
        )
        with session_scope(session_factory) as session:
            repository = SqlAlchemyProjectsRepository(session)
    ...
    def archive_project(
        ...
        return self._set_project_lifecycle(
            actor=actor,
    ...
    def remove_member(
        ...
        *,
        ...
        with session_scope(session_factory) as session:
            ...
            if (
                role == ProjectRole.ADMIN
                and repository.count_project_admins(project_id=project_uuid) <= 1
            ):
                raise ConflictError(
            ...
            repository.remove_project_member(
                project_id=project_uuid, user_id=target_user_id
            )

    def get_access(self, *, actor: ActorContext, project_id: str) -> ProjectAccess:
        session_factory = self._require_session_factory()
        project_uuid = parse_uuid(project_id, code="invalid_project_id")
        with session_scope(session_factory) as session:
            repository = SqlAlchemyProjectsRepository(session)
            ...
            if actor.principal_type == "user":
                ...
                role = SqlAlchemyServiceAccountsRepository(
                    session
                ).get_project_grant_role(
            ...
            return ProjectAccess(
                project_id=project_id, roles=roles, permissions=permissions
            )
    ...
    def takeover(
        ...
        *,
        ...
        self._policy_engine.require(
            actor=actor,
            authorization=AuthorizationRequest(
                permission=PermissionCode.PLATFORM_PROJECT_TAKEOVER
            ),
        )
        return self._restore_admin(project_id=project_id, user_id=actor.user_id)
    ...
    def restore_admin(
        ...
        *,
        ...
        return self._restore_admin(project_id=project_id, user_id=command.user_id)
