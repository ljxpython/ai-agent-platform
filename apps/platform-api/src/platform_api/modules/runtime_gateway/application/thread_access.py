...
def is_manager(actor: ActorContext, project_id: str) -> bool:
    return (
        ...
        and (
            actor.has_platform_role("platform_super_admin")
            or actor.has_project_role(project_id, "project_admin")
        )
    )
...
def is_owner(actor: ActorContext, metadata: dict) -> bool:
    return bool(
        ...
        and actor.user_id
        and metadata.get("owner_user_id") == actor.user_id
    )


def private_owner(actor: ActorContext, metadata: dict) -> bool:
    return (
        metadata.get("access_version") == 1
        and metadata.get("visibility") == "private"
        and is_owner(actor, metadata)
    )
...
def personal_memory_allowed(metadata: dict, *, project_id: str, user_id: str) -> bool:
    return (
        ...
        and metadata.get("visibility") == "private"
        and not metadata.get("shared_actions")
        and not metadata.get("project_actions")
    )
...
def initial_metadata(actor: ActorContext, metadata: dict) -> dict:
    ...
    result.update(
        access_version=1,
        owner_user_id=actor.user_id if personal else None,
        visibility="private" if personal else "project",
        shared_actions={},
        takeovers={},
...
def allowed(actor: ActorContext, project_id: str, metadata: dict, action: str) -> bool:
    ...
    if not actor.project_role_set(project_id) and not (
        action == "read" and is_manager(actor, project_id)
    ):
        return False
    ...
    if action == "read" and is_manager(actor, project_id):
        ...
        if (
            ...
            and isinstance(takeover.get("expires_at"), (int, float))
            and takeover["expires_at"] > time.time()
        ):
            return True
    if metadata.get("visibility") == "project":
        return bool(actor.project_role_set(project_id)) and (
            action in metadata.get("project_actions", [])
            or is_manager(actor, project_id)
        )
...
def require_action(
    actor: ActorContext, project_id: str, metadata: dict, action: str
) -> None:
    if not allowed(actor, project_id, metadata, action):
        raise ForbiddenError(
            code="thread_action_denied", message="Thread action is not permitted"
        )
...
def record_metadata(row: ThreadAccessRecord) -> dict:
    return {
        ...
        "shared_actions": row.shared_actions,
        "project_actions": row.project_actions,
...
def remove(factory, *, actor: ActorContext, project_id: str, thread_id: str) -> None:
    with session_scope(factory) as session:
        ...
        _audit(
            ...
            reason="Authorized Thread deletion",
        )


def remove_pending(factory, *, project_id: str, thread_id: str) -> bool:
    with session_scope(factory) as session:
        row = session.get(ThreadAccessRecord, thread_id, with_for_update=True)
        if (
            row is None
...
def visible_records(
    factory, *, actor: ActorContext, project_id: str
) -> dict[str, dict]:
    with session_scope(factory) as session:
        ...
        if actor.principal_type == "user" and actor.user_id:
            candidates.extend(
                [
                    ...
                    ThreadAccessRecord.shared_actions[actor.user_id]
                    .as_string()
                    .is_not(None),
                ]
            )
            if is_manager(actor, project_id):
                candidates.append(
                    ThreadAccessRecord.takeovers[actor.user_id].as_string().is_not(None)
                )
        # ponytail: JSON predicates scan project rows; normalize shares if large-project query latency requires indexes.
        rows = session.scalars(
            select(ThreadAccessRecord).where(
                ThreadAccessRecord.project_id == project_id, or_(*candidates)
            )
        )
        return {
            row.thread_id: record_metadata(row)
            for row in rows
            if allowed(actor, project_id, record_metadata(row), "read")
        }
    ...
    *,
    ...
    session.add(
        AuditLogRecord(
            ...
            result="success",
            method="POST",
            path=f"/api/langgraph/threads/{thread_id}/access",
            status_code=200,
            duration_ms=0,
...
def audit_takeover_access(
    factory, *, actor: ActorContext, project_id: str, thread_id: str
) -> None:
    with session_scope(factory) as session:
        _audit(
            session,
            ...
            project_id=project_id,
            thread_id=thread_id,
            action="thread.takeover.accessed",
            reason="time-limited private Thread access",
        )
...
def share(
    ...
    *,
    ...
    if (
        ...
        or set(actions) - SHARE_ACTIONS
        or (actions and "read" not in actions)
    ):
        raise BadRequestError(
            code="invalid_share_actions",
            message="Shared actions must be distinct known actions and include read",
        )
    with session_scope(factory) as session:
        row = session.get(ThreadAccessRecord, thread_id, with_for_update=True)
        metadata = record_metadata(row) if row else {}
        require_action(actor, project_id, metadata, "share")
        if not is_owner(actor, metadata) and not (
            row.visibility == "project" and is_manager(actor, project_id)
        ):
            current = set(metadata.get("shared_actions", {}).get(actor.user_id, []))
            if user_id is None or set(actions) - current:
                raise ForbiddenError(
                    code="share_delegation_denied",
                    message="Cannot delegate more actions than granted",
                )
        if user_id is None:
            if set(actions) - {"read", "comment", "edit"}:
                raise BadRequestError(
                    code="invalid_project_share",
                    message="Project sharing supports read, comment and edit only",
                )
            ...
            if member is None and actions:
                raise ForbiddenError(
                    code="share_project_member_required",
                    message="Share target must be a current project member",
                )
            if "delete" in actions and (
                member is None or member.value != "project_admin"
            ):
                raise ForbiddenError(
                    code="share_delete_denied",
                    message="Only the owner or a project/platform administrator may delete",
                )
            ...
            if actions:
                if user_id not in grants and len(grants) >= 100:
                    raise BadRequestError(
                        ...
                        message="A Thread supports at most 100 individual shares",
                    )
        ...
        _audit(
            ...
            project_id=project_id,
            thread_id=thread_id,
            action="thread.share.updated",
            reason=f"target={user_id or 'project'}; actions={','.join(actions)}",
        )
        session.flush()
        return record_metadata(row)
...
def takeover(
    ...
    *,
    ...
    if not is_manager(actor, project_id):
        raise ForbiddenError(
            code="thread_takeover_denied", message="Administrator permission required"
        )
    if (
        category not in {"security_incident", "compliance", "user_support", "handover"}
        or not 1 <= duration_minutes <= 60
        or len(reason.strip()) < 10
        or len(reference.strip()) < 3
    ):
        raise BadRequestError(
            code="invalid_takeover",
            message="Provide a supported category, reason, case reference and a 1–60 minute duration",
        )
    with session_scope(factory) as session:
        row = session.get(ThreadAccessRecord, thread_id, with_for_update=True)
        ...
        grants = {
            key: value
            for key, value in row.takeovers.items()
        ...
        _audit(
            ...
            project_id=project_id,
            thread_id=thread_id,
            action="thread.takeover.granted",
            reason=f"{category}; {reference}; {reason}; expires_at={grant['expires_at']}",
        )
...
def end_takeover(
    ...
    with session_scope(factory) as session:
        ...
        _audit(
            ...
            project_id=project_id,
            thread_id=thread_id,
            action="thread.takeover.ended",
            reason="Administrator ended temporary access",
        )
