...
def _resolve_action(
    *,
    ...
    if len(segments) >= 2 and segments[:2] == ["api", "projects"]:
        ...
        if (
            len(segments) == 4
            and segments[3] == "member-candidates"
            and method == "GET"
        ):
    ...
    if len(segments) >= 2 and segments[:2] == ["api", "announcements"]:
        ...
        if len(segments) == 3 and segments[2] == "feed" and method == "GET":
            return (
                ...
                "announcement_feed",
                _resolve_project_id(request),
            )
        if len(segments) == 3 and segments[2] == "read-all" and method == "POST":
            return (
                "announcement.feed.read_all_marked",
                "announcement_feed",
                _resolve_project_id(request),
            )
        if len(segments) == 3 and method == "PATCH":
        ...
        if len(segments) == 4 and segments[3] == "read" and method == "POST":
            return (
                "announcement.item.read_marked",
                "announcement",
                clean_str(segments[2]),
            )
    ...
    if (
        segments == ["_system", "platform-config", "feature-flags"]
        and method == "PATCH"
    ):
        return (
    ...
    if len(segments) >= 2 and segments[:2] == ["api", "service-accounts"]:
        ...
        if (
            len(segments) == 5
            and segments[3] == "project-grants"
            and method == "DELETE"
        ):
            return (
                "service_account.project_grant.removed",
                "project",
                clean_str(segments[4]),
            )
    ...
    if (
        len(segments) >= 4
        and segments[:2] == ["api", "graphs"]
        and segments[3] == "assistant-parameter-schema"
    ):
        if method == "GET":
    ...
    if len(segments) >= 2 and segments[:2] == ["api", "runtime"]:
        ...
        if segments[2:] == ["models", "refresh"] and method == "POST":
            return (
                "runtime.model.collection.refreshed",
                "runtime_model",
                _resolve_project_id(request),
            )
        if segments[2:] == ["tools"] and method == "GET":
            return (
                "runtime.tool.collection.listed",
                "runtime_tool",
                _resolve_project_id(request),
            )
        if segments[2:] == ["tools", "refresh"] and method == "POST":
            return (
                "runtime.tool.collection.refreshed",
                "runtime_tool",
                _resolve_project_id(request),
            )
        if segments[2:] == ["graphs"] and method == "GET":
            return (
                "runtime.graph.collection.listed",
                "runtime_graph",
                _resolve_project_id(request),
            )
        if segments[2:] == ["graphs", "refresh"] and method == "POST":
            return (
                "runtime.graph.collection.refreshed",
                "runtime_graph",
                _resolve_project_id(request),
            )
    ...
    if (
        ...
        and segments[3] == "runtime-policies"
    ):
        project_id = clean_str(segments[2])
        ...
        if len(segments) == 6 and segments[4] == "graphs" and method == "PUT":
            return (
                "runtime.policy.graph.updated",
                "runtime_policy_graph",
                clean_str(segments[5]),
            )
    ...
    if len(segments) >= 2 and segments[:2] == ["api", "langgraph"]:
        ...
        if (
            len(segments) == 6
            and segments[2] == "threads"
            and segments[4:6] == ["stream", "events"]
            and method == "POST"
        ):
