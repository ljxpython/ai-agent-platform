...
from platform_api.core.errors import BadRequestError, ConflictError, NotAuthenticatedError, NotFoundError, ServiceUnavailableError
from platform_api.core.identifiers import parse_uuid
from platform_api.modules.audit.domain import AuditResult
from platform_api.modules.iam.application import AuthorizationRequest, IamPolicyEngine, PermissionCode
from platform_api.modules.operations.application.audit import write_operation_audit_event
from platform_api.modules.operations.application.artifacts import LocalOperationArtifactStore, OperationArtifact
...
from platform_api.modules.operations.application.execution import with_actor_snapshot
from platform_api.modules.operations.application.ports import OperationDispatcherProtocol, StoredOperation
from platform_api.modules.operations.domain import (
...
class OperationsService:
    ...
    def _resolve_actor_for_project(
        self,
        *,
        ...
        if not project_id or not actor.is_authenticated or not actor.user_id:
            return actor
        if project_id in actor.project_roles:
            return actor
        try:
            ...
            return actor
        role = SqlAlchemyProjectsRepository(uow.session).get_project_member_role(
            project_id=project_uuid,
            user_id=user_uuid,
        )
        if role is not None:
            new_project_roles = dict(actor.project_roles)
            new_project_roles[project_id] = (role.value,)
            return replace(actor, project_roles=new_project_roles)
        return actor
    ...
    async def get_operation(
        ...
        *,
        ...
        async with SqlAlchemyUnitOfWork(session_factory) as uow:
            ...
            effective_actor = self._resolve_actor_for_project(
                uow=uow,
                actor=actor,
                project_id=operation.project_id,
            )
            self._require_read_access(actor=effective_actor, project_id=operation.project_id)
            return self._operation_view(operation)
    ...
    async def cancel_operation(
        ...
        *,
        ...
        async with SqlAlchemyUnitOfWork(session_factory) as uow:
            ...
            self._require_write_access(actor=effective_actor, project_id=operation.project_id)
