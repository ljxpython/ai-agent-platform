...
class SqlAlchemyAnnouncementsRepository:
    ...
    def list_admin_announcements(
        ...
        *,
        ...
        if scope_type and scope_type.strip():
            base_stmt = base_stmt.where(
                AnnouncementRecord.scope_type == scope_type.strip()
            )
        if scope_project_id is not None:
            base_stmt = base_stmt.where(
                AnnouncementRecord.scope_project_id == scope_project_id
            )

        stmt = (
            base_stmt.order_by(
                desc(AnnouncementRecord.publish_at), desc(AnnouncementRecord.created_at)
            )
            .offset(offset)
            .limit(limit)
        )
    ...
    def mark_all_announcements_read(
        ...
        *,
        ...
        changed = 0
        for item in self.list_visible_announcements(
            user_id=user_id, project_id=project_id, now=now
        ):
            if item.is_read:
                continue
