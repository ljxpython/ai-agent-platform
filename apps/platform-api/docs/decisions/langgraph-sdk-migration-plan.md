...
- [x] `apps/platform-api` 已补 `langgraph-sdk` 依赖
...
当前 `platform-api` 的问题，不在模块拆分，而在上游接入层重复造轮子：

- 旧 `platform-api` 已经有一套比较成熟的 `langgraph_sdk` 接入方式。
- `platform-api` 又重新写了一套 `httpx` 代理：
...
这里说的“改 SDK / 保留 HTTP”，指的是 `platform-api` 内部调用 LangGraph 上游的方式，不是对外 platform endpoint 要改路径。
...
1. 在 `apps/platform-api/pyproject.toml` 增加 `langgraph-sdk` 依赖。
...
当前 `platform-api` 最大的问题之一，不是代码风格，而是这块几乎没有成体系自动化测试。SDK 化之前必须先列好测试面。
...
- `platform-api/runtime_gateway + assistants adapter` 可以改成 SDK 版。
- 而且不是“可选优化”，而是下一阶段值得做的基础重构。
- 但要注意：
  - 改的是“LangGraph 上游适配层”
  - 不是推翻 `platform-api` 的模块化单体结构
