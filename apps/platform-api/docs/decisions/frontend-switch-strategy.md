...
- 后端改造的主战场是 `apps/platform-api`

## 1. 为什么前端不复制一份

如果再建一份新的 `platform-web`，会立刻出现：
...
- 页面组件不关心是 `platform-api` 还是 `platform-api`
...
- `assistants` 模块级 control plane 选择现在正式挂到 `v2` runtime 开关下：
  - runtime 开关开启且 v2 token 存在时，`mode: 'runtime'` 的 assistants 请求会优先走 `platform-api`
...
- `threads` 页面现在和 `chat` 对齐目标展示：
  ...
  - 老线程如果只有 ID，会受控查询 `platform-api` 的 assistants / runtime graph catalog 补名字
  - 新 thread 在创建时会写入 `target_display_name / assistant_name / graph_name`，避免后续继续制造裸 ID 元数据
- `identity` 模块现在支持显式 `runtime` 模式：
  - 登录仍保持 legacy 主登录链路，但会在登录成功后引导建立 `platform-api` 会话
...
- `announcements` 管理页现在支持显式 `runtime` 模式：
  ...
  - 公告列表 / 创建 / 更新 / 删除优先走 `platform-api`
  - 顶栏公告中心 feed 现已一并切到 `platform-api`，与公告管理页保持同一套控制面
...
- `runtime_catalog` 已在 `platform-api` 落地：
...
- `runtime.service` 已具备模块级 control plane 选择能力：
  - runtime 开关开启且 v2 token 存在时，models / tools 自动走 `platform-api`
  - 否则自动回退 legacy 的 `/_management/runtime/*`
- `runtime` 页面族现在也纳入 runtime 项目上下文：
  - `/workspace/runtime`
  - `/workspace/runtime/models`
  - `/workspace/runtime/tools`
- `graphs` 页面与 `assistant create` 的 graph 目录现在优先读取 `platform-api` 的 runtime graph catalog
- `assistants` 列表页与详情页已经完成 `platform-api` 联调：
...
- `users` 模块现在支持显式 `runtime` 模式：
  - `users` 模块已纳入 control plane 选择器，`runtime` 开关开启且 `v2` token 存在时，`/workspace/users*` 会优先使用 `platform-api`
...
- `testcase` 模块现在支持显式 `runtime` 模式：
  - `testcase` 模块已纳入 control plane 选择器，`runtime` 开关开启且 `v2` token 存在时，`/workspace/testcase*` 会优先使用 `platform-api`
...
- `assistant create` 与 chat 运行参数已经不再读取 legacy runtime 目录，而是直接消费 `platform-api` 的 runtime catalog
- 当前 graph 目录不再依赖 `runtime info -> graphs/search/count` 的能力探测链路
- 只有当 `platform-api` 不可用或 graph catalog 请求失败时，前端才受控回退 legacy 的 `/_management/catalog/graphs`
...
- 只有当 runtime 项目上下文可独立工作后，才允许打开 `VITE_PLATFORM_API_RUNTIME_ENABLED=true`
...
> 保持 `platform-web` 为唯一正式宿主，通过服务层和模块契约逐步切换到新的控制面。

## 7. 下一批推荐切换

当前下一批最值得继续切到 `platform-api` 的前端模块：
