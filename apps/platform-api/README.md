...
仅在 `.env` 不存在时复制，已有环境保留自己的凭据。本地开发统一使用本机独立 PostgreSQL，示例关闭自动建表；首次建库和历史数据迁移见[数据库运维规范](../../docs/guides/database-operations.md)。先按[配置说明](docs/handbook/configuration.md)设置登录密钥、管理员密码、Runtime地址与委托；模型配置需要有效Fernet master key。
