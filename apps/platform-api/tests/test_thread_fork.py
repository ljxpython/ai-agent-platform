...
class ThreadForkTest(unittest.IsolatedAsyncioTestCase):
    async def test_fork_copies_checkpoint_values_into_a_new_thread(self) -> None:
        ...
        service._load_thread = AsyncMock(return_value={"metadata": {"project_id": "p", "graph_id": "showcase_demo", "access_policy": "workspace_write", "agent_id": "agent-123"}})  # type: ignore[method-assign]

        result = await service.fork_thread(
            actor=SimpleNamespace(), project_id="p", thread_id="source",
    ...
    async def test_fork_removes_target_when_state_copy_fails(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "broken"):
            await service.fork_thread(actor=SimpleNamespace(), project_id="p", thread_id="source", checkpoint_id="checkpoint", title=None)
